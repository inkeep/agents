...
async function setupProjectInDatabase(skipDocker) {
  ...
  // Step 1: Start databases (skip if --skip-docker flag is set)
  if (skipDocker) {
    logStep(
      1,
      'Skipping docker database startup. Please ensure that INKEEP_AGENTS_MANAGE_DATABASE_URL and INKEEP_AGENTS_RUN_DATABASE_URL environment variables are configured for your cloud databases'
    );
  } else {
