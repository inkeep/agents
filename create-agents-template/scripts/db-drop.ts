...
async function deleteDatabase() {
  const runDatabaseUrl = process.env.INKEEP_AGENTS_RUN_DATABASE_URL;
  const manageDatabaseUrl = process.env.INKEEP_AGENTS_MANAGE_DATABASE_URL;

  if (!manageDatabaseUrl) {
    console.error('❌ INKEEP_AGENTS_MANAGE_DATABASE_URL environment variable is not set');
    process.exit(1);
  }

  if (!runDatabaseUrl) {
    console.error('❌ INKEEP_AGENTS_RUN_DATABASE_URL environment variable is not set');
    process.exit(1);
  }
