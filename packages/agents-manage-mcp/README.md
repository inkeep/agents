...
## Table of Contents
<!-- $toc-max-depth=2 -->
* [@inkeep/agents-manage-mcp](#inkeepagents-manage-mcp)
...
[![Install MCP Server](https://cursor.com/deeplink/mcp-install-dark.svg)](cursor://anysphere.cursor-deeplink/mcp/install?name=InkeepAgentsManage&config=eyJtY3BTZXJ2ZXJzIjp7Iklua2VlcEFnZW50c01hbmFnZSI6eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyJAaW5rZWVwL2FnZW50cy1tYW5hZ2UtbWNwIiwic3RhcnQiLCItLWNvb2tpZS1hdXRoIiwiLi4uIiwiLS1iZWFyZXItYXV0aCIsIi4uLiJdfX19)
...
```bash
claude mcp add @inkeep/agents-manage-mcp npx @inkeep/agents-manage-mcp start -- --cookie-auth ... --bearer-auth ...
```
...
```bash
npx @inkeep/agents-manage-mcp start --cookie-auth ... --bearer-auth ...
```

For a full list of server arguments, run:

```
npx @inkeep/agents-manage-mcp --help
```
...
2. Run `npm install`
3. Run `npm run build`
4. Run `node ./bin/mcp-server.js start --cookie-auth ... --bearer-auth ...`
...
```bash
npx @modelcontextprotocol/inspector node ./bin/mcp-server.js start --cookie-auth ... --bearer-auth ...
```
...
### MCP Server Created by [Speakeasy](https://www.speakeasy.com/?utm_source=@inkeep/agents-manage-mcp&utm_campaign=mcp-typescript)
