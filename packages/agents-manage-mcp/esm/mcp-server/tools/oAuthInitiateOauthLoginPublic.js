...
export const tool$oAuthInitiateOauthLoginPublic = {
...
Detects OAuth requirements and redirects to authorization server (public endpoint)`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await oAuthInitiateOauthLoginPublic(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
