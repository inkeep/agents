/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { contextConfigGetContextConfigById } from "../../funcs/contextConfigGetContextConfigById.js";
import { GetContextConfigByIdRequest$zodSchema } from "../../models/getcontextconfigbyidop.js";
import { formatResult } from "../tools.js";
const args = {
    request: GetContextConfigByIdRequest$zodSchema,
};
export const tool$contextConfigGetContextConfigById = {
    name: "context-config-get-context-config-by-id",
    description: `Get Context Configuration`,
    annotations: {
        "title": "",
        "destructiveHint": false,
        "idempotentHint": false,
        "openWorldHint": false,
        "readOnlyHint": true,
    },
    args,
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await contextConfigGetContextConfigById(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        const value = result.value;
        return formatResult(value, apiCall);
    },
};
//# sourceMappingURL=contextConfigGetContextConfigById.js.map