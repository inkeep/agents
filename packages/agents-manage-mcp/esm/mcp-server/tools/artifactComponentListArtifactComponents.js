/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { artifactComponentListArtifactComponents } from "../../funcs/artifactComponentListArtifactComponents.js";
import { ListArtifactComponentsRequest$zodSchema } from "../../models/listartifactcomponentsop.js";
import { formatResult } from "../tools.js";
const args = {
    request: ListArtifactComponentsRequest$zodSchema,
};
export const tool$artifactComponentListArtifactComponents = {
    name: "artifact-component-list-artifact-components",
    description: `List Artifact Components`,
    annotations: {
        "title": "",
        "destructiveHint": false,
        "idempotentHint": false,
        "openWorldHint": false,
        "readOnlyHint": true,
    },
    args,
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await artifactComponentListArtifactComponents(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        const value = result.value;
        return formatResult(value, apiCall);
    },
};
//# sourceMappingURL=artifactComponentListArtifactComponents.js.map