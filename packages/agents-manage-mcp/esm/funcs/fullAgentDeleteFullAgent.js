...
import { DeleteFullAgentRequest$zodSchema, DeleteFullAgentResponse$zodSchema, } from "../models/deletefullagentop.js";
 ...
 */
export function fullAgentDeleteFullAgent(client$, request, options) {
    return new APIPromise($do(client$, request, options));
}
async function $do(client$, request, options) {
    const parsed$ = safeParse(request, (value$) => DeleteFullAgentRequest$zodSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
    ...
    const [result$] = await M.match(M.nil(204, DeleteFullAgentResponse$zodSchema), M.json(400, DeleteFullAgentResponse$zodSchema, {
