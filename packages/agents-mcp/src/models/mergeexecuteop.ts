/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: 05967ad7a3cf
 */

import * as z from "zod";
import { BadRequest, BadRequest$zodSchema } from "./badrequest.js";
import { ErrorResponse, ErrorResponse$zodSchema } from "./errorresponse.js";
import { Forbidden, Forbidden$zodSchema } from "./forbidden.js";
import {
  InternalServerError,
  InternalServerError$zodSchema,
} from "./internalservererror.js";
import {
  MergeExecuteRequest,
  MergeExecuteRequest$zodSchema,
} from "./mergeexecuterequest.js";
import {
  MergeExecuteResponse,
  MergeExecuteResponse$zodSchema,
} from "./mergeexecuteresponse.js";
import { NotFound, NotFound$zodSchema } from "./notfound.js";
import { Unauthorized, Unauthorized$zodSchema } from "./unauthorized.js";
import {
  UnprocessableEntity,
  UnprocessableEntity$zodSchema,
} from "./unprocessableentity.js";

export type MergeExecuteRequestRequest = {
  tenantId: string;
  projectId: string;
  body?: MergeExecuteRequest | undefined;
};

export const MergeExecuteRequestRequest$zodSchema: z.ZodType<
  MergeExecuteRequestRequest
> = z.object({
  body: MergeExecuteRequest$zodSchema.optional(),
  projectId: z.string().describe("Project identifier"),
  tenantId: z.string().describe("Tenant identifier"),
});

export type MergeExecuteResponseResponse = {
  ContentType: string;
  StatusCode: number;
  RawResponse: Response;
  MergeExecuteResponse?: MergeExecuteResponse | undefined;
  BadRequest?: BadRequest | undefined;
  Unauthorized?: Unauthorized | undefined;
  Forbidden?: Forbidden | undefined;
  NotFound?: NotFound | undefined;
  ErrorResponse?: ErrorResponse | undefined;
  UnprocessableEntity?: UnprocessableEntity | undefined;
  InternalServerError?: InternalServerError | undefined;
};

export const MergeExecuteResponseResponse$zodSchema: z.ZodType<
  MergeExecuteResponseResponse
> = z.object({
  BadRequest: BadRequest$zodSchema.optional().describe("Bad Request"),
  ContentType: z.string().describe(
    "HTTP response content type for this operation",
  ),
  ErrorResponse: ErrorResponse$zodSchema.optional().describe(
    "Conflict — stale hashes, concurrent merge, or unresolved conflicts",
  ),
  Forbidden: Forbidden$zodSchema.optional().describe("Forbidden"),
  InternalServerError: InternalServerError$zodSchema.optional().describe(
    "Internal Server Error",
  ),
  MergeExecuteResponse: MergeExecuteResponse$zodSchema.optional().describe(
    "Merge executed successfully",
  ),
  NotFound: NotFound$zodSchema.optional().describe("Not Found"),
  RawResponse: z.custom<Response>(x => x instanceof Response).describe(
    "Raw HTTP response; suitable for custom response parsing",
  ),
  StatusCode: z.int().describe("HTTP response status code for this operation"),
  Unauthorized: Unauthorized$zodSchema.optional().describe("Unauthorized"),
  UnprocessableEntity: UnprocessableEntity$zodSchema.optional().describe(
    "Unprocessable Entity",
  ),
});
