/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: decfdcdceaed
 */

import { subAgentsDeleteSubAgentExternalAgentRelation } from "../../funcs/subAgentsDeleteSubAgentExternalAgentRelation.js";
import { DeleteSubAgentExternalAgentRelationRequest$zodSchema } from "../../models/deletesubagentexternalagentrelationop.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: DeleteSubAgentExternalAgentRelationRequest$zodSchema,
};

export const tool$subAgentsDeleteSubAgentExternalAgentRelation: ToolDefinition<
  typeof args
> = {
  name: "sub-agents-delete-sub-agent-external-agent-relation",
  description: `Delete Sub Agent External Agent Relation`,
  annotations: {
    "title": "",
    "destructiveHint": true,
    "idempotentHint": false,
    "openWorldHint": false,
    "readOnlyHint": false,
  },
  args,
  tool: async (client, args, ctx) => {
    const [result] = await subAgentsDeleteSubAgentExternalAgentRelation(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    return formatResult(result.value);
  },
};
