...
vi.mock('@inkeep/agents-core', () => ({
  ...
  findWorkAppSlackWorkspaceByTeamId: vi.fn(() => vi.fn()),
  ...
  listAgents: vi.fn(() => vi.fn()),
  listProjectsPaginated: vi.fn(() => vi.fn()),
}));
...
describe('Event Utils', () => {
  ...
  describe('getThreadContext', () => {
    ...
    it('should return empty string when only one message', async () => {
      const mockClient = {
        conversations: {
          replies: vi.fn().mockResolvedValue({
            messages: [{ user: 'U123', text: 'Single message' }],
          }),
        },
      };
...
describe('getWorkspaceDefaultAgent', () => {
  ...
  it('should return Nango default when available', async () => {
...
describe('getChannelAgentConfig', () => {
  ...
  it('should fall back to workspace default when no channel config', async () => {
    ...
    const { findWorkspaceConnectionByTeamId, getWorkspaceDefaultAgentFromNango } = await import(
      '../nango'
    );
