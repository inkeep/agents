...
const expectSecurityRejection = async (expression: string, expectedPattern: string) => {
  await expect(
    JsonTransformer.transform({}, expression)
  ).rejects.toThrow(new RegExp(expectedPattern, 'i'));
};

const expectValidTransformation = async (data: any, expression: string) => {
  await expect(
    JsonTransformer.transform(data, expression)
...
const createComplexTestData = () => ({
  ...
  items: [
    createTestItem('apple', 1.20),
    createTestItem('banana', 0.50),
    createTestItem('cherry', 2.00)
  ]
});
...
describe('JsonTransformer', () => {
  ...
  describe('Security Validation', () => {
    ...
    describe('expression length validation', () => {
      it('should reject expressions longer than maximum length', async () => {
        ...
        await expect(
          JsonTransformer.transform({}, longExpression)
    ...
    describe('syntax validation', () => {
      it('should reject invalid JMESPath syntax', async () => {
        ...
        for (const expression of invalidExpressions) {
          await expect(
            JsonTransformer.transform({}, expression)
          ).rejects.toThrow(/Invalid JMESPath syntax/);
        }
      });
  ...
  describe('Timeout Protection', () => {
    it('should have timeout protection mechanism', async () => {
      ...
      await expect(
        JsonTransformer.transform(data, 'simple', { timeout: 5000 })
      ).resolves.toBe('value');
      
      // Test that very short timeouts are also accepted (even if they don't timeout in practice)
      await expect(
        JsonTransformer.transform(data, 'simple', { timeout: 1 })
      ).resolves.toBe('value');
    ...
    it('should use default timeout when not specified', async () => {
      ...
      await expect(
        JsonTransformer.transform(data, 'test')
      ).resolves.toBe('value');
    });
  });
  ...
  describe('Basic Functionality', () => {
    ...
    it('should handle complex JMESPath operations when validation allows', async () => {
      ...
      await expect(
        JsonTransformer.transform({}, complexExpression)
      ).rejects.toThrow(/Invalid JMESPath syntax/);
  ...
  describe('transformWithConfig', () => {
    ...
    it('should throw error when neither config is provided', async () => {
      ...
      await expect(
        JsonTransformer.transformWithConfig(data, config)
      ).rejects.toThrow(/Either jmespath or objectTransformation must be provided/);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should provide detailed error messages', async () => {
      await expect(
        JsonTransformer.transform({}, 'invalid.[syntax')
      ).rejects.toThrow(/Invalid JMESPath syntax/);
    });

    it('should handle null and undefined inputs gracefully', async () => {
      await expect(
        JsonTransformer.transform(null, 'data')
      ).resolves.toBeNull();
      
      await expect(
        JsonTransformer.transform(undefined, 'data')
      ).resolves.toBeNull();
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle circular references safely', async () => {
      ...
      await expect(
        JsonTransformer.transform(data, 'name')
      ).resolves.toBe('test');
    });
  });
  ...
  describe('Security Regression Tests', () => {
    ...
    it('should prevent prototype pollution attempts', async () => {
      ...
      await expect(
        JsonTransformer.transform(data, '__proto__.polluted')
      ).rejects.toThrow(/dangerous pattern/);
