...
describe('workAppSlack data access', () => {
  ...
  describe('User Mapping CRUD', () => {
    ...
    it('should update last used timestamp', async () => {
      ...
      expect(mapping.lastUsedAt).toBeNull();

      await updateWorkAppSlackUserMappingLastUsed(db)(TEST_TENANT_ID, TEST_USER_ID, TEST_TEAM_ID);

      const updated = await findWorkAppSlackUserMapping(db)(
        TEST_TENANT_ID,
        TEST_USER_ID,
        TEST_TEAM_ID
      );
