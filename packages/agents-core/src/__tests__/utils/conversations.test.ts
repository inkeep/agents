...
describe('deriveRelationId', () => {
  ...
  it('should be deterministic — same inputs always produce the same ID', () => {
    const id1 = deriveRelationId('tenant1', 'project1', 'agent1', 'sub1', 'tool1');
    const id2 = deriveRelationId('tenant1', 'project1', 'agent1', 'sub1', 'tool1');
    const id3 = deriveRelationId('tenant1', 'project1', 'agent1', 'sub1', 'tool1');
    expect(id1).toBe(id2);
    expect(id2).toBe(id3);
  });
  ...
  it('should produce different IDs when argument order changes', () => {
    const id1 = deriveRelationId('a', 'b');
    const id2 = deriveRelationId('b', 'a');
    expect(id1).not.toBe(id2);
  });
});
