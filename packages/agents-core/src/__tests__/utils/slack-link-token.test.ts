...
describe('slack-link-token', () => {
  ...
  describe('isSlackLinkToken', () => {
    it('should return true for a valid Slack link token', async () => {
      const token = await signSlackLinkToken(validParams);
      expect(isSlackLinkToken(token)).toBe(true);
    });

    it('should return false for an invalid token', () => {
      expect(isSlackLinkToken('not.a.token')).toBe(false);
    });

    it('should return false for a token with wrong issuer', () => {
      const wrongIssuerToken =
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ3cm9uZy1pc3N1ZXIiLCJ0b2tlblVzZSI6InNsYWNrTGlua0NvZGUifQ.test';
      expect(isSlackLinkToken(wrongIssuerToken)).toBe(false);
    });

    it('should return false for a token with wrong tokenUse', () => {
      const wrongTokenUse =
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJpbmtlZXAtYXV0aCIsInRva2VuVXNlIjoic2xhY2tVc2VyIn0.test';
      expect(isSlackLinkToken(wrongTokenUse)).toBe(false);
    });

    it('should return false for a non-JWT string', () => {
      expect(isSlackLinkToken('regular-api-key')).toBe(false);
    });
  });
