...
// ============================================================================
// WORK APP CONFIGURATION TABLES
// ============================================================================

export type WorkAppType = 'slack' | 'teams';

export type WorkAppConfigMetadata = {
  configuredByUserId?: string;
  configuredByUserName?: string;
  notes?: string;
};

export const workAppConfigs = pgTable(
  'work_app_configs',
  {
    ...tenantScoped,
    appType: varchar('app_type', { length: 50 }).notNull().$type<WorkAppType>(),
  ...
  (table) => [
    ...
    unique('work_app_configs_workspace_channel_unique').on(
      table.tenantId,
      table.appType,
      table.workspaceId,
      table.channelId
...
export const workAppConfigsRelations = relations(workAppConfigs, ({ one }) => ({
  project: one(projects, {
