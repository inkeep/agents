...
import { createTestConfigDatabaseClientNoMigrations } from './test-config-client';
...
export function createAgentsManageDatabaseClient(config: AgentsManageDatabaseConfig): AgentsManageDatabaseClient {
  let connectionString = config.connectionString || process.env.AGENTS_MANAGE_DATABASE_URL;

  if (env.ENVIRONMENT === 'test') {
    return createTestConfigDatabaseClientNoMigrations();
  }
