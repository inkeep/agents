...
describe('Credential Environment Settings System', () => {
  ...
  describe('Environment Setting Helpers', () => {
    ...
    it('should provide type-safe helpers for single environment', async () => {
      ...
      const { getEnvironmentSettingKeys, hasEnvironmentSetting, getEnvironmentSetting } =
        createEnvironmentSettings({ test });

      expect(getEnvironmentSettingKeys().sort()).toEqual(['api-key', 'oauth-token']);
    ...
    it('should handle empty environments gracefully', () => {
      ...
      const { getEnvironmentSettingKeys, hasEnvironmentSetting } = createEnvironmentSettings({
        empty,
      });
    ...
    it('should throw errors for missing environment settings', async () => {
      ...
      await expect(getEnvironmentSetting('missing-environment-setting' as any)).rejects.toThrow(
    ...
    it('should automatically infer environment names from object keys', async () => {
      ...
      const { getEnvironmentSetting } = createEnvironmentSettings({ local, staging });
  ...
  describe('Edge Cases and Error Scenarios', () => {
    ...
    it("should error when NODE_ENV doesn't match any environment name", async () => {
      ...
      const { getEnvironmentSetting } = createEnvironmentSettings({ production });
