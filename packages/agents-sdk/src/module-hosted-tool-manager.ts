...
export class ModuleHostedToolManager {
  ...
  async deployInlineTool(toolFunction: InlineToolFunction): Promise<ModuleHostedToolServer> {
  ...
  private async generateModuleFiles(server: ModuleHostedToolServer): Promise<void> {
...
`;
  ...
  private async handleFunctionExecution(message: any, childProcess: ChildProcess): Promise<void> {
  ...
  private async waitForServerReady(server: ModuleHostedToolServer, maxRetries = 10): Promise<void> {
