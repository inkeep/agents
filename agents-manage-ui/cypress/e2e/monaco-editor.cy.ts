...
describe('Monaco Editor', () => {
  it('should update the SubAgent prompt editor when switching nodes', () => {
    ...
    cy.contains('You are a helpful assistant responsible for converting location').should('be.visible');

    // Assert 2nd node
    cy.get('.react-flow__node').contains('Weather forecaster').click();
    cy.contains('You are a helpful assistant responsible for taking in coordinates').should('be.visible');
