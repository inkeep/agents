...
export function MCPBreakdownCard({ conversation }: MCPBreakdownCardProps) {
  const mcpStats = useMemo(() => {
    ...
    for (const activity of mcpToolCalls) {
      ...
      const hasError = activity.status === ACTIVITY_STATUS.ERROR || activity.status === ACTIVITY_STATUS.WARNING;

      if (!serverToolMap.has(serverName)) {
        serverToolMap.set(serverName, new Map());
      }

      const toolMap = serverToolMap.get(serverName)!;
      if (!toolMap.has(toolName)) {
        toolMap.set(toolName, { successes: 0, failures: 0 });
      }
