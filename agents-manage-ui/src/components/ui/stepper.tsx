...
function StepperItem({ className, ...props }: ComponentProps<'div'>) {
  return <div className={cn('group/step relative flex gap-3', className)} {...props} />;
}
...
function StepperContent({ className, ...props }: ComponentProps<'div'>) {
  return <div className={cn('flex-1 space-y-3 pb-6 group-last/step:pb-0', className)} {...props} />;
}

function StepperTitle({ className, ...props }: ComponentProps<'p'>) {
  return (
    <p className={cn('font-medium text-[15px] leading-6 text-foreground', className)} {...props} />
  );
}
