...
const editCredentialFormSchema = z.object({
	name: z
		...
		.refine(
			(val) => val.length > 0,
			"Name cannot be empty after transformation",
		)
				...
				<div className="space-y-8">
					...
					<div className="space-y-3">
						...
						<Input
							type="text"
							disabled={true}
							value={getCredentialType(credential)}
						/>
						{getCredentialType(credential) === "Bearer Auth" && (
							<div className="text-xs text-muted-foreground p-3 bg-muted/30 rounded-md">
								<p className="mb-2">
									<strong>How this works:</strong> When your agent connects to
									the MCP server, this API key will be automatically sent as an
