...
import {
	AddNodeCommand,
	AddPreparedEdgeCommand,
} from "@/features/graph/commands/commands";
import {
...
} from "@/lib/utils/graph-error-parser";
...
interface GraphProps {
	graph?: FullGraphDefinition & {
		contextConfig?: Partial<
			Pick<ContextConfig, "id" | "name" | "description">
		> & {
...
function Flow({
	...
	const { nodeId, edgeId, setQueryState, openGraphPane, isOpen } =
		useSidePane();
		...
		setInitial(
			...
			graphEdges,
			{
				...
				contextConfig: {
					...
					contextVariables:
						formatJsonField(graph?.contextConfig?.contextVariables) || "",
					requestContextSchema:
						formatJsonField(graph?.contextConfig?.requestContextSchema) || "",
				},
			},
			dataComponentLookup,
			artifactComponentLookup,
	...
	const onConnectWrapped = useCallback((params: Connection) => {
		...
		if (isSelfLoop) {
			...
			(targetHandle === agentNodeTargetHandleId ||
				targetHandle === agentNodeSourceHandleId)
		) {
			newEdge = {
				...newEdge,
				type: EdgeType.A2A,
	...
	const onSelectionChange = useCallback(
		({ nodes, edges }: { nodes: Node[]; edges: Edge[] }) => {
			...
			const edge =
				edges.length === 1 &&
				(edges[0]?.type === EdgeType.A2A ||
					edges[0]?.type === EdgeType.SelfLoop)
					? edges[0]
					: null;
	...
	const onSubmit = useCallback(async () => {
		...
		if (res.success) {
			...
			try {
				...
				toast.error(
					summaryMessage || "Failed to save graph - validation errors found",
				);
			} catch (parseError) {
