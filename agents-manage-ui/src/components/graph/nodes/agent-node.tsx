...
export function AgentNode(props: NodeProps & { data: AgentNodeData }) {
	...
	const dataComponentLookup = useGraphStore(
		(state) => state.dataComponentLookup,
	);
	const artifactComponentLookup = useGraphStore(
		(state) => state.artifactComponentLookup,
	);
	...
	const dataComponentNames = useMemo(
		() =>
			data?.dataComponents
				?.map((id: string) => dataComponentLookup[id]?.name)
				.filter(Boolean) || [],
