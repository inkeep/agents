...
function MetadataEditor() {
	...
	const graphUrl = `${AGENTS_RUN_API_URL}/api/chat`;
		...
		<div className="space-y-8">
				...
				<div className="space-y-2">
					<div className="text-sm leading-none font-medium flex items-center gap-1">
						...
						<Tooltip>
							...
							<TooltipContent>
								Use this endpoint to chat with your graph or connect it to the
								Inkeep widget via the graphUrl prop. Supports streaming
								responses with the Vercel AI SDK data stream protocol.
							</TooltipContent>
						</Tooltip>
					</div>
			...
			<div className="space-y-2">
				...
				<p className="text-xs text-muted-foreground">
					System-level prompt that defines the intended audience and overall
					goal of this graph. Applied to all agents.
				</p>
			</div>
			<Separator />
			...
			<div className="space-y-4">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-1">
						Set default models that will be inherited by agents that don't have
						their own models configured.
					</p>
				</div>
				...
				<div className="text-xs text-muted-foreground p-3 bg-blue-50 dark:bg-blue-950/20 rounded-md border border-blue-200 dark:border-blue-800">
					...
					<ul className="space-y-1 text-blue-800 dark:text-blue-200">
						<li>
							• <strong>Models</strong>: Project → Graph → Agent (partial
							inheritance - missing models only)
						</li>
						<li>
							• <strong>Individual model types</strong> inherit independently
							(base, structuredOutput, summarizer)
						</li>
						<li>
							• <strong>Explicit settings</strong> always take precedence over
							inherited values
						</li>
						<li>
							• <strong>Provider options</strong> are inherited along with the
  ...
  "maxTokens": 1024
			...
			<div className="space-y-4">
				...
				<div className="space-y-2">
					...
					/>
					<p className="text-xs text-muted-foreground">
						Maximum number of agent transfers per conversation (defaults to 10
						if not set)
					</p>
				</div>
				...
				<div className="text-xs text-muted-foreground p-3 bg-blue-50 dark:bg-blue-950/20 rounded-md border border-blue-200 dark:border-blue-800">
					...
					<ul className="space-y-1 text-blue-800 dark:text-blue-200">
						<li>
							• <strong>transferCountIs</strong>: Project → Graph only (controls
						...
						<li>
							• <strong>Default fallback</strong>: transferCountIs = 10 if no
							value is set anywhere
						</li>
						<li>
							• <strong>Graph scope</strong>: This limit applies to all agents
							within this graph
						</li>
					</ul>
				</div>
			...
			<div className="space-y-4">
				<div>
					...
					<p className="text-xs text-muted-foreground mt-1">
						Configure structured status updates for conversation progress
						tracking.
					</p>
				</div>
				...
				<div className="space-y-4">
					<div className="space-y-2">
						<Label htmlFor="status-updates-enabled">
					...
					{(statusUpdates?.enabled ?? true) && (
						<>
							<div className="space-y-2">
								<Label htmlFor="status-updates-prompt">
							...
							<div className="space-y-4">
								...
								{statusUpdates?.numEvents && (
									<div className="space-y-2">
										...
										/>
										<p className="text-xs text-muted-foreground">
											Number of events/steps between status updates (default:
											10)
										</p>
									</div>
								)}
								...
								{statusUpdates?.timeInSeconds && (
									<div className="space-y-2">
										...
										/>
										<p className="text-xs text-muted-foreground">
											Time interval in seconds between status updates (default:
											30)
										</p>
									</div>
								)}
									...
									placeholder={`[
  {
    ...
    "schema": {
      ...
      "required": ["tool_name", "summary"]
								...
								/>
								<p className="text-xs text-muted-foreground">
									Define structured components for status updates. Each
									component has an id, name, description, and JSON schema.
								</p>
							</div>
						</>
					)}
