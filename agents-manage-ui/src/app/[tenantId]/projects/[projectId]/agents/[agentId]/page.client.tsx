...
export const Agent: FC<AgentProps> = ({
  ...
  toolLookup = {},
  credentialLookup = {},
  skillLookup: initialSkillLookup = {},
}) => {
  ...
  const {
    ...
    edges,
    metadata,
    skillLookup: skillLookupState,
  } = useAgentStore((state) => ({
    storeNodes: state.nodes,
    edges: state.edges,
    metadata: state.metadata,
    skillLookup: state.skillLookup,
  }));
  const {
    setNodes,
    setEdges,
  ...
  const refreshAgentGraph = useCallback(
    async (options?: { fetchTools?: boolean }) => {
      ...
      try {
        ...
        const updatedSkillLookup = fullProject.skills || {};
        ...
        setInitial(
          ...
          updatedSkillLookup as Record<string, Skill>
        );
