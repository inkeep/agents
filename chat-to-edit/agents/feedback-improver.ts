import { agent, trigger } from '@inkeep/agents-sdk';
import { z } from 'zod';
...
const feedbackImproveTrigger = trigger({
  ...
  description: 'Trigger feedback improvement workflow from the feedback page',
...
export const feedbackImprover = agent({
  ...
  triggers: () => [feedbackImproveTrigger],
...
The user may have pre-selected evaluation resources:
...
The Feedback Analyst AUTOMATICALLY runs dataset items through the agent, then triggers a batch evaluation — no asking the user first. If datasets and evaluators were pre-selected, use those specific resources.
