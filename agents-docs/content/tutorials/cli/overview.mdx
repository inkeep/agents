---
title: CLI Tutorials
sidebarTitle: Overview
description: Learn how to use the Inkeep CLI for common workflows
icon: LuTerminal
---

The Inkeep CLI is your primary tool for managing agents, syncing configurations, and deploying to Inkeep Cloud. These tutorials walk you through common CLI workflows.

## What you'll learn

<Cards>
  <Card title="Set Up a CLI Profile" icon="LuUserCog" href="/tutorials/cli/setup-profile">
    Create a profile and authenticate with a remote deployment (only needed for cloud push/pull)
  </Card>
  <Card title="Push to Inkeep Cloud" icon="LuCloudUpload" href="/tutorials/cli/push-to-cloud">
    Push your local agent configurations to Inkeep Cloud
  </Card>
  <Card title="Pull from Inkeep Cloud" icon="LuCloudDownload" href="/tutorials/cli/pull-from-cloud">
    Pull agent configurations from Inkeep Cloud to your local project
  </Card>
</Cards>

## Prerequisites

Before starting these tutorials, ensure you have:

- The Inkeep CLI installed globally:

```bash
npm install -g @inkeep/agents-cli
```

- Access to [Inkeep Cloud](https://inkeep.com/cloud-waitlist) or a [self-hosted deployment](/deployment/vercel)

## Quick reference

For a complete list of CLI commands and options, see the [CLI Reference](/typescript-sdk/cli-reference).
