...
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│   PR Created ──────► Vercel Preview Deployment                   │
│                      (pr-123-<project>.vercel.app)               │
│                                                                  │
│   PR Merged to main ──────► Vercel Preview Deployment (Staging)  │
│                             (main-<project>.vercel.app)          │
│                                                                  │
│   GitHub Release Published ──────► GitHub Action                 │
│                                    │                             │
│                                    ├──► Deploy to Preview        │
│                                    ├──► Wait for Checks          │
...
### Step 1: Disable Automatic Production Deployments

By default, Vercel deploys to production when you push to your production branch (usually `main`). To separate staging from production, disable automatic production deployments:

1. Go to **Project Settings > Git** in the Vercel Dashboard
2. Under **Production Branch**, change the value to `_disabled_`
3. Save your changes

<Note>
  Setting the Production Branch to `_disabled_` is a special Vercel value that prevents any branch from automatically triggering production deployments. All pushes, including to `main`, will create preview deployments instead.
</Note>

After this configuration:
- Pushes to `main` create preview deployments at `main-<project>.vercel.app`
- Pushes to PR branches create preview deployments at `<branch>-<project>.vercel.app`
- **No branch** triggers automatic production deployments

### Step 2: Configure GitHub Actions for Production Releases
...
### Step 4: Optional Staging Domain Configuration
...
2. Click **Add Domain**
3. Enter your staging domain (e.g., `staging.yourdomain.com`)
...
|--------|--------|---------|
| `staging.yourdomain.com` | `main` | Staging environment for testing |
| `yourdomain.com` | Production | Production (deployed via GitHub releases) |
...
2. **Test on staging**: Verify changes at `staging.yourdomain.com` or `main-<project>.vercel.app`
...
<Accordions>
  <Accordion title="Troubleshooting Staging/Production Deployments">
    ...
    <ul>
      <li>Verify Production Branch is set to <code>_disabled_</code> in Project Settings > Git</li>
    ...
    <ul>
      <li>Verify the domain is configured with Git Branch set to <code>main</code></li>
