...
## Fluid Compute Database Pooling

When deploying to [Vercel Fluid Compute](https://vercel.com/docs/functions/fluid-compute), Postgres connection pools are automatically registered with Vercel's `attachDatabasePool` API. This allows Vercel to manage idle database clients before serverless functions suspend, improving connection reuse and reducing the risk of leaked idle clients across suspended function instances.

This applies to both the manage (Doltgres) and runtime (Postgres) database pools. No additional configuration is required — the framework detects the Vercel environment automatically and attaches the pools.
