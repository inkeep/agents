...
# Optional: set to 'false' to disable Inkeep-attached Anthropic prompt caching
# INKEEP_PROMPT_CACHING_ENABLED=false

# Optional: ALTCHA Sentinel bot protection (all three required together, or all unset)
# INKEEP_SENTINEL_API_KEY_ID=
# INKEEP_SENTINEL_API_KEY_SECRET=
# INKEEP_SENTINEL_BASE_URL=

# Optional: Legacy PoW compatibility (both required together; migration-only)
# INKEEP_SENTINEL_V1_API_KEY_ID=
# INKEEP_SENTINEL_V1_API_KEY_SECRET=
```
...
| `INKEEP_PROMPT_CACHING_ENABLED`       | (Optional) Set to `false` to disable Inkeep-attached Anthropic prompt caching globally. Defaults to `true`. See [Prompt Caching](/guides/observability/prompt-caching) for details.                                                        |
| `INKEEP_SENTINEL_API_KEY_ID`          | (Optional) ALTCHA Sentinel API key ID for bot protection. Required with `INKEEP_SENTINEL_API_KEY_SECRET` and `INKEEP_SENTINEL_BASE_URL`.                                                       |
| `INKEEP_SENTINEL_API_KEY_SECRET`      | (Optional) ALTCHA Sentinel API key secret. Required with `INKEEP_SENTINEL_API_KEY_ID` and `INKEEP_SENTINEL_BASE_URL`.                                                                          |
| `INKEEP_SENTINEL_BASE_URL`            | (Optional) ALTCHA Sentinel service base URL. Required with `INKEEP_SENTINEL_API_KEY_ID` and `INKEEP_SENTINEL_API_KEY_SECRET`.                                                                  |
| `INKEEP_SENTINEL_V1_API_KEY_ID`       | (Optional) Legacy PoW compatibility API key ID. For migration only; reuses `INKEEP_SENTINEL_BASE_URL`.                                                                                         |
| `INKEEP_SENTINEL_V1_API_KEY_SECRET`   | (Optional) Legacy PoW compatibility API key secret. For migration only.                                                                                                                        |
