...
# Cross-subdomain authentication (optional, required when UI and API don't share a 3-part parent domain)
AUTH_COOKIE_DOMAIN=
TRUSTED_ORIGIN=
```
...
| `NANGO_SERVER_URL`                       | `https://api.nango.dev`                                                                                                                                            |
| `AUTH_COOKIE_DOMAIN`                     | (Optional) Shared cookie domain for cross-subdomain auth. Required when UI and API domains don't share a 3-part parent (e.g., `app.inkeep.com` + `api.agents.inkeep.com`). Set to the common root domain with a leading dot (e.g., `.inkeep.com`). |
| `TRUSTED_ORIGIN`                         | (Optional) Trusted origin for CORS. Required with `AUTH_COOKIE_DOMAIN`. Set to your Manage UI URL (e.g., `https://app.inkeep.com`). |
