...
- For `--ui`: requires authentication (same as push/pull). The component file is written with an export ensured on the main component. Register the component in your chat widget with the dashboard component **name** as the key and your imported component as the value (e.g. `components: { "Weather F": WeatherFore }`).
