...
<Tip>
  When pass criteria are configured and an evaluation fails, webhook destinations subscribed to `evaluation.failed` receive notifications with the failing conditions and actual vs expected values. See [Outbound Webhooks](/visual-builder/outbound-webhooks#evaluation-events) to configure destinations.
</Tip>
