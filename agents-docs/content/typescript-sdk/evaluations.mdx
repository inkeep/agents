...
| `runAsUserIds` | string[] | No | User IDs whose identities are used for scheduled runs (enables per-user fan-out) |
| `scheduleConfig` | object | No | Configuration for automatic scheduled execution (see below) |
| `scheduleConfig.cronExpression` | string | No | 5-field cron expression for recurring runs (mutually exclusive with `runAt`) |
| `scheduleConfig.cronTimezone` | string | No | IANA timezone for cron evaluation (defaults to `UTC`) |
| `scheduleConfig.runAt` | string | No | ISO 8601 timestamp for a one-time scheduled run (mutually exclusive with `cronExpression`) |
| `scheduleConfig.enabled` | boolean | No | Whether the schedule is active (defaults to `true`) |
| `scheduleConfig.dispatchDelayMs` | number | No | Milliseconds to stagger dispatch per user for load spreading |
...
// { datasetRunId: "...", datasetRunIds: ["..."], status: "pending", totalItems: 3 }
```
...
|--------|------|----------|-------------|
| `evaluatorIds` | string[] | No | Evaluator IDs to use (overrides config defaults) |
| `branchName` | string | No | Branch name for the run |
| `runAsUserId` | string | No | User ID whose identity is used for this run |
| `dispatchDelayMs` | number | No | Milliseconds to stagger dispatch per user |
...
Run configurations can be scheduled to run automatically on a cron schedule or at a specific future time. Add a `scheduleConfig` when creating a run configuration:

```typescript
const scheduledRunConfig = await client.createDatasetRunConfig({
  ...
  runAsUserIds: ["user_abc123"], // Optional: run as specific users
});
```

Scheduled runs support retry with exponential backoff and idempotency. One-time schedules (`runAt`) automatically disable after completion.

When `runAsUserIds` is set, the system fans out the run per user — each user gets their own dataset run executed with their identity and credentials.

For more details on scheduling options, see [Scheduled Triggers](/typescript-sdk/triggers/scheduled).
...
| Method | Purpose |
|--------|---------|
| `createDatasetRunConfig({ name, datasetId, agentIds?, evaluatorIds?, scheduleConfig?, runAsUserIds? })` | Create a run configuration (agents, evaluators, optional schedule) |
| `triggerDatasetRun(runConfigId, { evaluatorIds?, branchName?, runAsUserId?, dispatchDelayMs? }?)` | Start a run; returns `datasetRunId`, `datasetRunIds`, `status`, `totalItems` |
