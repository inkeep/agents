...
| `passCriteria` | object | No | Nested conditions with `and`/`or` operators. Conditions can include nested groups for complex logic. Supports numeric operators (`>`, `<`, `>=`, `<=`, `=`, `!=`) and boolean operators (`=`, `!=`). Max nesting depth: 5. |
