...
<Callout>
  **Note**: When providing a `props` schema, every property must include a non-empty `description` for LLM compatibility.
</Callout>
...
props: z.object({
  temperature: z.number().describe("Current temperature"), // ❌ Cannot extract number from text
  condition: z.string().describe("Weather condition"), // ❌ Cannot extract condition from text
});
...
**No schema?** For artifact components, you can omit `props` to save the entire tool result without filtering.
