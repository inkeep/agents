---
title: Data Components in TypeScript SDK
sidebarTitle: Custom UI Blocks
description: Define data component schemas for rich, interactive agent messages using the TypeScript SDK.
icon: LuBlocks
---
...
The widget also injects a `messageId` prop automatically (see [Injected props](#injected-props) below).

```tsx
const TaskList = ({ tasks, totalCount, completedCount, messageId }) => (
...
In addition to the props defined in your schema, the widget automatically injects a `messageId` prop (a `string`) into every data component. This identifies the message the component belongs to, which is useful for attributing analytics events to a specific conversation message.

Because data components render in isolated React roots, React context cannot reach them. `messageId` is passed via props so that in-component event handlers can reference the parent message without a context provider.
