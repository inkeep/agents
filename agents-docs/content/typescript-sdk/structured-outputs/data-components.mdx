...
<Callout type="warning">
  **Required**: The `props` field is mandatory for data components. Additionally, every property in your schema must include a non-empty `description` for LLM compatibility.
</Callout>
...
<Callout>
JSON Schema is also supported. When using JSON Schema, ensure every property includes a `"description"` field.
</Callout>
