...
```
activities-planner-complex/
├── index.ts
├── agents/
│   └── activities-planner-complex.ts
└── skills/
    ├── weather-safety-guardrails/
    │   └── SKILL.md
    └── structured-itinerary-responses/
...
import path from 'node:path';
import { loadSkills, project } from '@inkeep/agents-sdk';
import { activitiesPlannerAgent } from './agents/activities-planner-complex';
...
```typescript title="agents/activities-planner-complex.ts"
