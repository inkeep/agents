---
title: Skills in TypeScript SDK
sidebarTitle: Skills
description: Attach reusable instruction blocks to sub-agents
icon: LuHexagon
keywords: skills, prompts, instructions, reuse, sub-agents, SKILL.md
---
...
<Note>The directory name must match the `name` field in the frontmatter.</Note>
...
<Tip>
The `description` is how the agent decides whether to load an on-demand skill, so make it specific and front-load the scenarios it applies to ("Use when…"). Keep it tight: descriptions are capped at **1024 characters**, and every always-loaded skill — plus the outline of each on-demand skill — sits in the system prompt on every turn, so verbose descriptions cost context. See [Tuning Agents](/guides/agent-engineering/tuning-agents#keep-tool-and-skill-descriptions-tight).
</Tip>

### Reserved skill names

<Callout type="warning">
Skill names beginning with `open-knowledge` (e.g. `open-knowledge`, `open-knowledge-write-skill`) are reserved for internal platform use. Attempts to create, rename, or push a skill with a reserved name will be rejected.
</Callout>
...
3. Recursively loads every file in each skill directory with paths relative to the skill root
4. Validates that the `name` matches the directory name
5. Returns an array of `SkillDefinition` objects
...
When `alwaysLoaded` is `false` or omitted, skills appear as an outline in the system prompt. The agent can load the full content and any synced nested files when needed using the built-in `load_skill` tool.
