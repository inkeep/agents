...
| **Mock** (testing) | `mock/default`<br/>`mock/any-name` | None required |
...
<Tip>
The `mock/` provider (`mock/default`, `mock/any-name`) returns deterministic responses without API calls or keys. Use it for testing and CI/CD. It supports both streaming and non-streaming and returns synthetic token usage.
</Tip>
