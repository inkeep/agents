...
If the SSE stream disconnects or the user refreshes the page while an approval is pending, poll the pending approvals endpoint to restore state:
...
const { hasPending, approval } = await response.json();

if (hasPending && approval) {
  // Restore the approval UI with:
  // - approval.toolCallId
  // - approval.toolName
  // - approval.args (the tool input)
  // - approval.isDelegated (true if delegated to another agent)
}
```

<Note>
  Pending approval recovery requires [Durable execution mode](/visual-builder/execution-modes#durable-mode). In Classic mode, approvals are request-scoped and lost on stream disconnection.
</Note>
...
- [Execution modes](/visual-builder/execution-modes)
