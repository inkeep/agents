---
title: Tool Approvals in TypeScript SDK
sidebarTitle: Approvals
description: Require explicit user approval before running selected tools using the TypeScript SDK.
icon: LuShield
keywords: tool approvals, user confirmation, tool safety, execution control, MCP tools
---

<SkillRule
  id="tool-approvals-config"
...
<Note>
  In [durable execution mode](#durable-execution-mode), the workflow **suspends** instead of holding the HTTP connection open. Approvals can be submitted via a dedicated REST endpoint, and the workflow automatically resumes after a response is received.
</Note>
...
When an agent runs with `executionMode: 'durable'`, tool approvals work differently from the default in-memory flow. Instead of holding the HTTP connection open while waiting for a response, the durable workflow **suspends** and frees server resources.

You can submit approvals via the dedicated REST endpoint:

```
POST /run/api/executions/:executionId/approvals/:toolCallId
```

The `executionId` is returned in the `x-workflow-run-id` response header when the execution starts. The `toolCallId` comes from the `tool-approval-request` event in the stream.
...
To deny a tool call, set `approved` to `false` and optionally include a `reason`:
...
curl -X POST "http://localhost:3002/run/api/executions/$EXECUTION_ID/approvals/$TOOL_CALL_ID" \
  ...
  -d '{
    "approved": false,
    "reason": "Use a different city"
  }'
```

After the approval is submitted, the workflow automatically resumes and continues execution. If your client was disconnected, you can reconnect to the stream using `GET /run/api/executions/:executionId/stream`.

See the [Chat API](/talk-to-your-agents/chat-api) for the full durable executions reference.
