...
In [durable execution mode](/typescript-sdk/execution-modes), tool approvals also work for tools called by delegated sub-agents. When a delegated sub-agent invokes a tool that requires approval, the approval request surfaces to the end user. Once the user approves or denies, the decision is passed back to the delegated sub-agent and execution resumes.
