...
When the agent calls `fetch-page` and then `extract-text`, it passes the HTML output directly into the next call. The intermediate HTML never surfaces to the user. When designing tools for sequential use, aligning return types with downstream input types makes pipelines more natural — though it isn't required, since agents can also select a specific field from a tool's output to pass forward.
...
- **Design for composability** - When tools form a pipeline, aligning return types with downstream input types makes chaining more natural, though agents can also pass a specific field from a tool's output forward
