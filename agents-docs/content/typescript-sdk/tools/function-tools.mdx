...
- **Tenki** - Uses [Tenki Sandbox](https://tenki.cloud) microVMs, a hosted sandbox provider that works from any deployment environment. Set `SANDBOX_TENKI_API_KEY` (a Tenki API key, `tk_...`) to enable it
...
const app = createAgentsApp({
  sandboxConfig: {
    provider: "native", // or 'vercel' / 'tenki' for hosted microVMs
...
For the Tenki provider, pass the API key (or set `TENKI_API_KEY` in the environment):
...
const app = createAgentsApp({
  sandboxConfig: {
    provider: "tenki",
    runtime: "node22",
    timeout: 60000, // per-command execution timeout
    vcpus: 2, // CPU cores per sandbox microVM
