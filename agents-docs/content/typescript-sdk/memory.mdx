...
icon: LuBrain
keywords: memory, context window, conversation history, message limits, token limits, delegation, tool results, images, binary files, binary attachments, media proxy
---
...
<SkillRule
  ...
  description="How conversation history is managed and included in context windows"
>
...
- **Binary files**: User-uploaded files (images, PDFs, etc.) and binary files returned by tools are persisted to blob storage and tracked as `binary_attachment` artifacts
...
## Binary Files in Memory

User-uploaded files (images, PDFs, etc.) and binary files returned by tools are automatically persisted to blob storage and registered as `binary_attachment` artifacts. This allows the conversation history to track binary content by reference rather than inline, keeping messages lightweight.

When conversation history is retrieved, blob references are resolved to secure media proxy URLs or model-consumable file parts (such as image data or file URLs), ensuring files remain accessible across sessions without exposing internal storage paths. Binary payloads (base64 blobs) are stripped from observability traces and logs to prevent bloat and accidental data leakage into telemetry.
</SkillRule>
