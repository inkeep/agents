...
<SkillRule
  id="agent-relationships-overview"
...
- In [durable execution mode](/execution-modes), delegated Sub Agents can use tools that require [approval](/typescript-sdk/tools/tool-approvals). The approval request surfaces to the user, and the decision is passed back to the delegated Sub Agent so it can continue execution
...
<SkillRule
  id="agent-relationships-when-to-use"
  skills="typescript-sdk"
