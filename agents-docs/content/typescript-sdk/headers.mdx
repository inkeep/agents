---
title: Headers in TypeScript SDK
sidebarTitle: Headers
description: Define and validate HTTP headers in the TypeScript SDK to pass dynamic context for personalized agent interactions.
icon: LuBoxes
...
When using [authenticated chat sessions](/talk-to-your-agents/app-credentials#authenticated-chat-sessions), non-standard claims from the customer-signed JWT are automatically extracted and made available as **verified claims** in the conversation context. Unlike headers and `userProperties`, verified claims are cryptographically verified and cannot be modified by the client.
...
const supportAgent = subAgent({
  id: "support-agent",
  name: "Support Agent",
  description: "Authenticated support assistant",
  prompt: `You are a support assistant.

  The user has been authenticated. Use their verified identity
  to provide personalized, account-specific assistance.

  Verified claims from their session are available in the conversation context
  and can be trusted for authorization decisions.`,
});
```
...
For setup instructions, see [Authenticated chat sessions](/talk-to-your-agents/app-credentials#authenticated-chat-sessions).
...
- [App Credentials](/talk-to-your-agents/app-credentials) - Authentication for chat components, including authenticated sessions
