...
<SkillRule
  id="external-agents-overview"
  skills="typescript-sdk"
  title="External Agents Overview"
  description="How to configure and use external agents using the A2A protocol"
>
...
const technicalSupportAgent = externalAgent({
  ...
  baseUrl: "https://api.example.com/agents/technical-support", // Base URL used for Agent Card discovery
});
```

The `baseUrl` should point at the agent's A2A base URL, not the raw RPC route. Inkeep fetches `/.well-known/agent.json` from that base URL and then uses the Agent Card's `url` field for RPC.

## Local Code Node for Development

For local development, you can expose a checked-out repository as an external coding agent with `inkeep code-node`:
...
Then register the local bridge like any other external agent:
...
The local code node publishes its Agent Card at `/.well-known/agent.json` and serves A2A requests from the service URL advertised by that card. Start the bridge with `--write` only if you want it to accept write-enabled coding requests.
...
const myExternalAgent = externalAgent({
  ...
  baseUrl: "https://api.example.com/agents/support", // A2A base URL
...
| `baseUrl`             | string              | Yes      | Base URL of the external A2A agent. Inkeep fetches `/.well-known/agent.json` from this URL and then uses the Agent Card's `url` field for RPC.              |
