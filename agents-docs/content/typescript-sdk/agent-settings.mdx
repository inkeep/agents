...
const customerSupportAgent = agent({
  ...
  executionMode: "classic", // or "durable"
...
Set the execution mode to control how the agent's execution lifecycle is managed:

```typescript
agent({
  id: "support-agent",
  executionMode: "durable", // or "classic" (default)
  defaultSubAgent: supportAgent,
});
```

- **`classic`** (default): Runs in a single in-process loop with minimal streaming latency (~100ms TTFB)
- **`durable`**: Wraps execution in a workflow with persistent checkpoints for crash recovery (~2s TTFB)

See [Execution Modes](/visual-builder/execution-modes) for detailed information about when to use each mode.
