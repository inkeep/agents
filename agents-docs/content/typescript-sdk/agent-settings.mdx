...
| `outputContract`     | object   | No       | Enforces structured output requirements on the sub-agent's response. See [Output Contracts](#output-contracts) |
...
Output contracts enforce structured output requirements on a sub-agent. Use them to ensure sub-agents emit specific data components, artifacts, or transfer control when required.

```typescript
const mySubAgent = subAgent({
  id: "structured-response-agent",
  name: "Structured Response Agent",
  prompt: "Always respond with structured data.",
  outputContract: {
    allowText: false,                    // Disable plain text responses
    requireComponent: ["customer-info"], // Must emit this data component
    requireArtifact: ["analysis-report"], // Must emit this artifact
    requireTransfer: false,              // Transfer not required
    onViolation: "retry",                // Retry on contract violation
  },
  dataComponents: [
    {
      id: "customer-info",
...
| ------ | ---- | ----------- |
| `allowText` | boolean | When `false`, the sub-agent cannot emit plain text responses. Default: `true` |
| `requireComponent` | string[] | Data component IDs that must appear in the response |
| `requireArtifact` | string[] | Artifact component IDs that must appear in the response |
| `requireTransfer` | boolean | When `true`, the sub-agent must transfer to another sub-agent |
| `onViolation` | string | Policy when the contract is violated: `"retry"`, `"error"`, or `"warn"`. Default: `"warn"` |

The output contract is surfaced in the sub-agent's system prompt, guiding the model to comply with the requirements.
