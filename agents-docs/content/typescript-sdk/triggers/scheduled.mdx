...
1. **Use specific cron expressions** - Avoid running too frequently; match the schedule to actual need
2. **Set appropriate timeouts** - Ensure `timeoutSeconds` is long enough for the agent to complete
3. **Configure retries for critical tasks** - Use `maxRetries` for important scheduled jobs
4. **Use `cronTimezone` for local schedules** - Avoid manually converting to UTC for recurring schedules
5. **Monitor invocations** - Check the invocation history for failed runs
6. **Use one-time triggers for deferred work** - Prefer `runAt` over cron for tasks that should execute exactly once
