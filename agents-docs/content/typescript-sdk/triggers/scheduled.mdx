...
When `runAsUserId` is set, the system resolves the user's profile timezone and uses it to populate the `x-inkeep-client-timezone` header during execution. This overrides the `cronTimezone` value for the execution context (the `cronTimezone` is still used for schedule calculation). This allows agents to receive the user's preferred timezone context during automated scheduled executions.
