...
When `runAsUserId` is set, agents automatically know your time based on your profile timezone. Note that `cronTimezone` is still used for schedule calculation, but the user's profile timezone takes precedence for the agent's time awareness during execution.
