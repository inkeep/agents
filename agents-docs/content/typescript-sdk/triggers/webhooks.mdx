---
title: Webhook Triggers in TypeScript SDK
sidebarTitle: Webhooks
description: Create webhook endpoints in the TypeScript SDK that allow external services to invoke your agents
icon: LuZap
---
...
| `runAsUserIds` | `string[]` | No | Array of user IDs whose identities and credentials are used during execution. Each user gets a separate invocation. |
| `dispatchDelayMs` | `number` | No | Optional delay in milliseconds between each per-user execution to smooth out downstream rate limits |
...
By default, webhook triggers execute without a user identity — tools with user-scoped credentials will fail. Setting `runAsUserIds` makes the trigger execute as one or more specific users, enabling access to their connected credentials (e.g., per-user OAuth tokens for GitHub, Slack, or Jira). Each selected user gets a separate execution and invocation record for every webhook request.
...
trigger({
  ...
  runAsUserIds: ["user_abc123", "user_def456"],
  dispatchDelayMs: 500, // 500ms delay between each user's execution
});
```

### Permission Rules

- Any user with `edit` permission can include themselves in `runAsUserIds`.
- Org admins and owners can include any user in the organization in `runAsUserIds`.
