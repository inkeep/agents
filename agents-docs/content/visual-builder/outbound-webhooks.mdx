...
<Steps>
  ...
  <Step>
    ...
    | **Evaluators** | No | Scope `evaluation.failed` events to specific evaluators. Leave empty to receive failures from all evaluators |
...
The `evaluation.failed` event type supports additional scoping by evaluator. When you select specific evaluators on a webhook destination, only failures from those evaluators trigger delivery. Leave the evaluators field empty to receive failures from all evaluators in the project.

In the webhook destination form, the evaluator scope appears under an **Advanced** collapsible section beneath the `evaluation.failed` event type checkbox.
