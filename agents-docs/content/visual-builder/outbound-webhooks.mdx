...
description: Receive real-time notifications via HTTP or Slack when conversations, feedback, or custom events occur in your project
...
Outbound webhooks send HTTP POST requests to your server or Slack channels when events happen in your project — new conversations, conversation updates, feedback submissions, and custom events. Use them to sync data with external systems, trigger downstream workflows, or build real-time integrations.
...
<Steps>
  ...
  <Step>
    ...
    | **Destination URL** | Conditional | The HTTPS endpoint that receives webhook POST requests. Required unless a Slack channel is selected. |
    | **Slack Channel** | Conditional | A Slack channel to post alerts to via the Inkeep bot. Required unless a destination URL is provided. See [Slack integration](#slack-integration). |
  ...
  <Step>
    ...
    After creating the destination, use the **Test** action to send a sample payload to your destination. The test request uses a `test` event type with synthetic data and a 10-second timeout, so you can verify your endpoint or Slack channel is reachable before real events fire.
  </Step>
</Steps>
...
| `conversation.tool.error` | A tool call fails. For MCP tools, only fires when all calls to the server fail in a generation |
...
You can send webhook events directly to a Slack channel using the Inkeep Slack bot instead of an incoming webhook URL. Slack destinations deliver rich formatted messages with Block Kit layouts.

<Note>
Slack channel destinations use the Slack Bot API (`chat.postMessage`) for delivery. The Inkeep bot must be installed in your workspace and added as a member of the target channel.
</Note>
...
<Steps>
  <Step>
    On the **Outbound Webhooks** page, click **Connect Slack**. If the Inkeep Slack app is not yet installed, you'll be guided through an OAuth flow to add it to your workspace.
  </Step>
  <Step>
    Select a channel from your workspace. The Inkeep bot must be a member of the channel — add it beforehand if needed.
  </Step>
  <Step>
    Configure your event types, agent scope, and other settings, then click **Create Destination**.
  </Step>
</Steps>

### Slack payload format

When delivering to a Slack channel, Inkeep posts messages via the Slack Bot API using Block Kit formatting:
...
The `text` field serves as the notification preview (mobile push, channel sidebar). The `blocks` array renders the rich card in the channel. Destinations using a Slack incoming webhook URL also receive this format. Non-Slack URL destinations always receive the standard JSON envelope without `text` or `blocks`.
...
The webhook destinations page shows all configured destinations with their name, destination (URL or Slack channel), enabled status, and subscribed event types.

| Action | Description |
| --- | --- |
| **Edit** | Update the name, destination, events, headers, agent scope, or enabled status |
...
For MCP tools, this event only fires when **all** calls to the same server fail during a generation. If at least one call to the server succeeds, the failures are treated as warnings and no webhook is sent.
