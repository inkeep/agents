...
If you already have an agent in the same project, open the **New Agent** dialog and choose
**Duplicate existing agent**.

Select the source agent, then provide a new name or ID for the copy. The duplicate keeps the
original agent's agent-scoped configuration and references the same project-scoped dependencies.
