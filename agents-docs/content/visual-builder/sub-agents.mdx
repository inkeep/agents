...
The **New Agent** dialog now has two starting points:

- **Create blank agent** starts from scratch.
- **Copy existing agent** lets you copy from the current project or another project.

When you choose **Copy existing agent**, the flow is the same in both cases:

1. Choose a **Source project**. The current project is selected by default.
2. Choose the **Source agent**.
3. Change the suggested name and ID.
4. Click **Copy agent**.

### Copy within the same project

If you keep **Source project** set to the current project, the copied agent stays within that
project.

The copy keeps the original agent's agent-scoped configuration and continues to reference the same
project-scoped dependencies. Webhook and scheduled triggers are not copied.

### Copy from another project

If you change **Source project** to a different project, the builder copies that agent into the
current project instead. Cross-project copies always read from the source project's main branch.

When you copy an agent across projects:

- Referenced project resources like MCP servers, external agents, functions, skills, data components, and artifact components are recreated in the target project when needed.
- Webhook and scheduled triggers are skipped.
- Team-agent relationships are not supported.
- Credentials are not copied. If a copied MCP server or external agent depends on credentials that are not available in the target project, it is added disconnected and needs to be reconnected after the copy.
...
| `model`               | string | No       | AI model identifier (e.g., "anthropic/claude-sonnet-4-20250514"). See [Model Configuration](/typescript-sdk/models)           |
| `providerOptions`     | object | No       | Model-specific configuration options (temperature, maxOutputTokens, etc.)                                                          |
| `fallbackModels`      | array  | No       | Ordered backup models if the primary fails. Requires [AI Gateway](/typescript-sdk/models#ai-gateway)                               |
| `allowedProviders`    | array  | No       | Restrict and prioritize which providers serve requests. Requires [AI Gateway](/typescript-sdk/models#ai-gateway)                    |
