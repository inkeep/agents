...
description: Multi-tenant authentication with organizations, team management, seat limits, and fine-grained project permissions
keywords: access control, authentication, authorization, roles, permissions, seat limits, entitlements
icon: LuShieldCheck
---
...
Enterprise cloud deployments may have seat limits that restrict the number of users per role:

| Limit Type | Counts |
|------------|--------|
| `seat:admin` | Owner + Admin users, plus pending Admin invitations |
| `seat:member` | Member users, plus pending Member invitations |

<Warning>
  Pending invitations count toward seat limits. If you're at capacity, you must remove a pending invitation or existing member before inviting someone new.
</Warning>

When a seat limit is reached, attempts to invite users or change roles return a `402 Payment Required` error with a structured error body containing the current usage and limit.

<Note>
  Self-hosted deployments do not enforce seat limits by default. Seat limits only apply when entitlement rows are configured for the organization.
</Note>
