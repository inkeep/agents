...
Pending invitations appear in the members table with a "Pending" status. Admins can manage these invitations via the actions dropdown menu:

| Action | Description |
|--------|-------------|
| **Copy invite link** | Copy the invitation link (email-password invitations only) |
| **Revoke invite** | Cancel the pending invitation |

Revoking an invitation displays a confirmation dialog before permanently removing the invite.
