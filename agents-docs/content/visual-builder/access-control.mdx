...
4. Enter one or more email addresses (comma-separated) and select an organization role (Admin or Member)
5. For **Member** invites, optionally select projects and a project role to grant access to when the invitee accepts
6. Click **Send Invitation**

<Note>
  The project selection is only available for the Member role. Admins automatically have full access to all projects.
</Note>
