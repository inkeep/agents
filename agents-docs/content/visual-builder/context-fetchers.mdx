---
title: Context Fetchers in Visual Builder
sidebarTitle: Context Fetchers
description: Set up context fetchers in the Visual Builder to pull real-time data from external APIs into agent prompts.
icon: LuCirclePlus
---
...
- **`credentialReferenceId`** (optional): The ID of a stored credential to use for authentication. The credential's secret value will be injected into the request headers at runtime. See [Finding the Credential Reference ID](#finding-the-credential-reference-id) below.
...
If your API requires authentication via a stored credential, add the `credentialReferenceId` field to your context variable definition:
