...
<Steps>
  ...
  <Step>
    ...
    - **Specific user(s)** — the trigger uses each selected user's connected credentials (e.g., OAuth tokens for GitHub, Slack, or Jira). When multiple users are selected, a single trigger is created that executes once per user on each tick.

    When assigning multiple users, you can optionally set **Dispatch Delay** (0–5000 ms) to stagger execution between users.

    <Note>
    Non-admin users can only assign triggers to themselves. Org admins can select any user in the organization and can assign multiple users to a single trigger.
    </Note>
  </Step>
  <Step>
    ### Choose a schedule
...
| Action | Description |
| --- | --- |
| **Run Now** | Execute the trigger immediately, regardless of its schedule. For multi-user triggers, creates one invocation per associated user. You can optionally target a single user via the dropdown. |
...
When a user's project access is revoked, they are automatically removed from all trigger-user associations in that project. If a trigger has zero associated users after removal, it is automatically disabled.
