...
<Steps>
  ...
  <Step>
    ...
    In the **Execution Identity** section, choose which user identities the trigger runs as. This determines whose credentials and permissions are used during execution.

    - **None** — the trigger runs without a user identity (tools requiring user-scoped credentials are skipped)
    - **Specific user(s)** — the trigger uses the selected user's connected credentials (e.g., OAuth tokens for GitHub, Slack, or Jira). When multiple users are selected, the trigger produces one independent execution per user at each scheduled tick.

    <Note>
    Non-admin users can only assign triggers to themselves. Org admins can select any user in the organization — and when creating a trigger, admins can select multiple users to create a single trigger that runs once per selected user.
    </Note>
  </Step>
  <Step>
    ### Choose a schedule
  ...
  <Step>
    ...
    You can also configure a **Dispatch Delay** (in the advanced settings) to stagger executions across users. This adds a delay between each user's execution to avoid hitting MCP tool rate limits. For example, a 1-second dispatch delay with 5 users staggers their executions over 4 seconds. The default is 0 (no delay).
  </Step>
  <Step>
    ### Save
...
The **Scheduled** tab shows all triggers in your project with their agent, schedule, status, last run, and next run. Multi-user triggers display a user count badge. Each trigger has a dropdown menu with the following actions:

| Action | Description |
| --- | --- |
| **Run Now** | Execute the trigger immediately, regardless of its schedule. For multi-user triggers, this creates one invocation per associated user. You can optionally target a single user. |
...
Each invocation shows its status, scheduled time, start time, duration, attempt number, the executing user, and a link to the conversation trace. For multi-user triggers, each scheduled tick produces one invocation per user. You can filter invocations by user to see per-user execution history.
