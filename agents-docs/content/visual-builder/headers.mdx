...
<Warning>
  All HTTP header keys are converted to **lowercase** before validation — for example, `Auth-Token` becomes `auth-token`. No other transformation is applied. Define your schema properties to match the lowercased key exactly. A mismatch will cause a `Context validation failed` error.
</Warning>
