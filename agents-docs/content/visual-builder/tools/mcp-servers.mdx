...
For programmatic access to tool health across all sub-agents, use the [Get Tool Status for Agent](/api-reference/agents#get-tool-status-for-agent) API endpoint.
...
If your MCP server requires authentication, you have three options: use the **"Click to Login"** button to authenticate via OAuth, manually create a credential, or dynamically pass credentials at runtime via headers.
...
### Dynamic credential pass-through

For in-product copilots where each end user has their own credentials for the MCP server, you can pass credentials dynamically at runtime via HTTP headers instead of storing a single static credential.

1. Register the MCP server with **no credential** selected (choose "No Authentication")
2. Define the credential as a required header in your agent's [headers schema](/visual-builder/headers)
3. On the connection between the MCP server and the Sub Agent, configure headers using `{{headers.*}}` templates to forward the value:
...
The template is resolved at runtime using the validated header from the incoming request and sent as an outbound header to the MCP server on every tool call.

<Note>
  Header keys are converted to **lowercase** (e.g., `Auth-Token` becomes `auth-token`). Define schema properties to match the lowercased key exactly. See [Headers — Forwarding Headers to MCP Servers](/visual-builder/headers#forwarding-headers-to-mcp-servers) for a full walkthrough.
</Note>
