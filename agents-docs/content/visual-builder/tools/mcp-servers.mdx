...
```json
{
  "Authorization": "Bearer {{headers.mcp_token}}"
}
```
...
<Note>
  Header keys are normalized to **lowercase**. Use lowercase property names in your headers schema (e.g., `mcp_token`, not `mcpToken`) and matching lowercase template variables. See [Headers — Forwarding Headers to MCP Servers](/visual-builder/headers#forwarding-headers-to-mcp-servers) for a full walkthrough.
</Note>
