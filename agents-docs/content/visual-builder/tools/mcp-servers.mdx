...
| `Credential`    | Authentication credential for the server. You must select an option: choose "No Authentication" if the server doesn't require authentication, or select a specific credential. For OAuth 2.1/PKCE flows, leave blank and use the `Click to Login` button in the server details page to authenticate |

<Warning>
  The **Credential** dropdown requires an active selection. If your server doesn't require authentication, you must explicitly select "No Authentication". If no selection is made, the server creation will not proceed.
</Warning>
