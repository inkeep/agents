...
<Warning>
  For MCP servers that do not use OAuth 2.1/PKCE authentication, you must select either **"No Authentication"** or choose an existing credential from the **Credential** dropdown. Leaving this field blank on non-OAuth servers will cause the server creation to hang.
</Warning>
