...
<Note>
  All HTTP header keys are converted to **lowercase** (e.g., `Auth-Token` becomes `auth-token`). Define schema properties to match the lowercased key exactly. See [Headers — Forwarding Headers to MCP Servers](/visual-builder/headers#forwarding-headers-to-mcp-servers) for a full walkthrough.
</Note>
