...
description: Understand the difference between user-level and project-level MCP servers and how they affect your team workflow
icon: LuUsers
keywords: MCP servers, user-level, project-level, authentication, team collaboration, credentials
---
...
- **Webhook triggers not supported**: User-level MCPs cannot be used with [webhook triggers](/talk-to-your-agents/triggers/webhooks) because they don't have user context. However, [scheduled triggers](/talk-to-your-agents/triggers/scheduled) can use user-level MCPs when configured with a `runAsUserId`.
