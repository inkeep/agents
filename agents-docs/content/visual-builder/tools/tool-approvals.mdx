...
<Tip>
  When an agent's **Execution mode** is set to **Durable**, the workflow suspends instead of holding the HTTP connection open. This means approvals can take as long as needed without timing out. In durable mode, approvals can also be submitted via the REST executions API. See [Tool Approvals (TypeScript SDK)](/typescript-sdk/tools/tool-approvals) for implementation details.
</Tip>
