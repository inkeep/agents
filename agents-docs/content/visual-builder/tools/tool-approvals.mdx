...
1. The agent pauses execution and emits a `tool-approval-request` event in the response stream. In [Durable mode](/visual-builder/execution-modes#durable-mode), the approval state is persisted to workflow metadata, making it recoverable after stream disconnection via the [pending approvals endpoint](/talk-to-your-agents/chat-api#recovering-pending-approvals).
