...
This same approval flow applies when a tool is called by a [delegated sub-agent](/typescript-sdk/sub-agents#delegation-relationships). In [durable execution mode](/visual-builder/agent-settings/execution-modes#durable-mode), the approval request bubbles up from the delegated sub-agent to the parent agent and is surfaced to the user. Once the user responds, the approval decision is forwarded back to the delegated sub-agent, which then executes or skips the tool accordingly.
