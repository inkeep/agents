---
title: Tool approvals in Visual Builder
sidebarTitle: Tool Approvals
description: Configure tool approval requirements in the Visual Builder to pause execution until a user approves the request.
icon: LuShield
keywords: tool approvals, user confirmation, tool safety, execution control, MCP tools, function tools, visual builder
---
...
If the SSE stream disconnects — due to network issues, a page reload, or a client crash — the `tool-approval-request` event may never reach the UI. The approval state is persisted server-side, so your client can recover it by polling a dedicated endpoint.

### Endpoint

`GET /run/v1/conversations/{conversationId}/pending-approvals`

This returns whether a pending tool approval exists for the conversation and, if so, the full approval details.
...
```json
{
  ...
  "approval": {
    ...
    "args": { "recordId": "rec_789" },
    "isDelegated": false,
    "workflowRunId": "wfr_xyz",
    "suspendedAt": "2026-04-07T22:45:00.000Z"
  }
}
```

When no approval is pending, the response is `{ "hasPending": false }`.

### When to poll

- **On page load** — check for any approval the user missed while away
- **After SSE reconnection** — recover approval state lost during a stream failure
- **Periodic polling** — as a fallback if your client doesn't maintain a persistent SSE connection
