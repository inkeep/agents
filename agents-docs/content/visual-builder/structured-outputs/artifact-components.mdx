...
3. For the props schema, you can either:
   - **Enter a JSON schema** with `inPreview: true` properties to define specific fields that will be immediately available. Every property must include a `"description"` field for LLM compatibility.
...
<Callout>
  **Note**: When providing a props schema, every property must include a non-empty `"description"` field for LLM compatibility.
</Callout>
...
// Note: Even with descriptions, this is wrong because the tool output doesn't match
