...
```tsx
const TaskList = ({ tasks, totalCount, messageId }) => (
...
In addition to the props defined in your JSON schema, the widget automatically injects a `messageId` prop identifying the message the component belongs to. This is useful for attributing analytics events back to a specific conversation message.

<Callout>
`messageId` is optional and additive — existing components that do not use it are unaffected.
</Callout>
