...
3. Enter a JSON schema defining the structure of your data component. **Every property must include a `"description"` field for LLM compatibility.**
4. Click "Submit".


<Callout type="warning">
  **Required**: The props schema is mandatory for data components. Your JSON schema must have `type: "object"` at the root level and every property must include a non-empty `"description"` field for LLM compatibility.
</Callout>
