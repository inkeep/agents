...
icon: LuFlaskConical
keywords: evaluations, evaluators, batch evaluations, continuous tests, test suites
---

The Visual Builder lets you define, manage, and run evaluations. You define evaluators (how to score agents), then run them in two ways: **batch evaluations** (one-time jobs over selected conversations) and **continuous tests** (automatic evaluation on a sample of live conversations). For **fixed test cases** (curated prompts and optional gold outputs), use **[test suites](/visual-builder/test-suites)** and test suite runs instead of batch evaluation over conversation history.
...
Evaluators can be scoped to specific agents. When an evaluator is scoped to one or more agents, it only runs on conversations handled by those agents during batch evaluations.

- **No agent scope** (default): The evaluator applies to all conversations regardless of which agent handled them.
- **Scoped to agents**: The evaluator only runs on conversations from the specified agents.

This is useful when you have agent-specific quality criteria. For example, a "Technical accuracy" evaluator might only apply to your documentation agent, while a "Tone" evaluator applies to all agents.

To configure agent scoping:
1. Open an evaluator from the **Evaluators** tab.
2. In the agent scope settings, select which agents this evaluator should apply to.
3. Leave empty to apply to all agents.

## Batch evaluations

Batch evaluations run selected evaluators over a set of conversations once. If evaluators are [scoped to specific agents](#agent-scoping), only evaluators matching each conversation's agent are applied. In the Visual Builder you scope the conversation set with a **date range**. The API and TypeScript SDK can also scope a job to specific **test suite runs** using test suite run IDs (`datasetRunIds` in the API)—for example to evaluate only conversations produced by a given [test suite](/visual-builder/test-suites) run. Starting a test suite run **with evaluators** creates a matching batch evaluation automatically.
...
For run-scoped jobs, use the [Evaluations API](/api-reference/evaluations) or [`triggerBatchEvaluation`](/typescript-sdk/evaluations) with `datasetRunIds`, or attach evaluators when you trigger a test suite run from **Test Suites**.
...
| **Test suites** | Curated items (input messages and optional expected output); run agents against them via run configurations. See [Test suites](/visual-builder/test-suites). |
