...
| **Test suites** | Curated items (input messages and optional expected output); run agents against them via run configurations. Run configurations can be [scheduled](/visual-builder/triggers/scheduled) to run automatically. See [Test suites](/visual-builder/test-suites). |
