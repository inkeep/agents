...
When an evaluator has pass criteria defined and an evaluation fails those criteria, you can receive real-time notifications via the `evaluation.failed` webhook event. Configure a webhook destination subscribed to this event type to trigger alerts, sync with external systems, or initiate remediation workflows.

See [Outbound Webhooks](/visual-builder/outbound-webhooks#evaluation-events) for setup and payload details.
