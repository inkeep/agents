...
API keys are the recommended approach for production server-to-server use, providing secure, scoped access to your Agents without exposing secrets to end-users. For app-based integrations such as web clients and authenticated end-user experiences, use [App Credentials](/talk-to-your-agents/app-credentials).
