...
**Triggers** allow external events or schedules to invoke your Agents automatically. Inkeep supports two types of triggers:

### Webhook Triggers

Webhook triggers create HTTP endpoints that allow external services to invoke your Agents. When a webhook is received, the payload is validated, transformed into a message, and used to start a new conversation.

Webhook triggers are useful for:
...
Scheduled triggers run agents on a time-based schedule using cron expressions or at a specific one-time date.

Scheduled triggers are useful for:
- **Recurring tasks** - Run daily reports, weekly summaries, or hourly health checks
- **One-time execution** - Schedule a migration task or maintenance job for a specific time
- **Automated workflows** - Generate content, send reminders, or process data on a schedule

Both trigger types support:
- **Message templates** - Transform trigger context into natural language messages using `{{placeholder}}` syntax
- **Retry configuration** - Configure retry behavior for failed invocations
- **Invocation tracking** - Monitor execution status and view run history

<Cards>
  <Card title="Webhook Triggers" icon="LuWebhook" href="/talk-to-your-agents/triggers">
    Learn how webhook triggers work and when to use them.
  </Card>
  <Card title="Scheduled Triggers" icon="LuClock" href="/typescript-sdk/scheduled-triggers">
    Create time-based triggers with cron expressions.
  </Card>
  <Card title="TypeScript SDK" icon="LuCode" href="/typescript-sdk/triggers">
...
<Card title="Triggers" icon="LuWebhook" href="/typescript-sdk/triggers">
