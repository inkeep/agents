...
<Cards>
  <Card title="Triggers" icon="LuZap" href="/talk-to-your-agents/triggers">
...
Each Agent has an **execution mode** that controls how its runs are managed:

- **Classic** (default) — The HTTP connection stays open for the entire request. Ideal for standard chat interactions and short-lived requests.
- **Durable** — Execution state is persisted in a workflow-backed system. The run can survive disconnections and long pauses (e.g. tool approvals that take minutes or hours). Clients receive an `x-workflow-run-id` header they can use to reconnect to the stream or check execution status via the Executions API.

Use **durable** mode when your agent relies on tool approvals that may not be answered immediately, or when you need the ability to reconnect to an in-progress execution.

<Note>
Execution mode is configured per agent in the [Visual Builder](/visual-builder/sub-agents) or [TypeScript SDK](/typescript-sdk/agent-settings). See the [Chat API](/talk-to-your-agents/chat-api) for streaming behavior details.
</Note>
