...
| `cache.intent.prefix_signature` | string | 10-character SHA-256 hex prefix of the cacheable prefix (system prompts and tool definitions) for debugging purposes |

### Cache state (consumer-derived, not a span attribute)

`cache_state` is **not emitted as a span attribute** — it is computed by consumers (Manage UI, the `pnpm cache-debug` CLI) from the attributes above:

| State | Derivation |
|-------|------------|
| `HIT` | `cache_read > 0` (a cache read is definitive proof of a hit) |
| `MISS` | `marker_count > 0` and `cache_read = 0` |
