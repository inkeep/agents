...
<Tip>
When running the OpenKnowledge desktop app from multiple git worktrees, dev instances are now automatically isolated per branch. The instance name is derived from your current git branch or worktree directory, so you can run multiple dev builds side-by-side without manual configuration. Set `OK_AUTO_INSTANCE=0` to disable this behavior, or use `OK_INSTANCE=<name>` for explicit naming.
</Tip>
