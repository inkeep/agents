...
<Tip>
If you use [fnm](https://github.com/Schniz/fnm) or [nvm](https://github.com/nvm-sh/nvm), run `fnm use` or `nvm use` in the repository root before installing dependencies. This reads `.node-version` and switches to the correct Node version automatically. The repository enforces strict version checking — `pnpm install` will fail if your Node version doesn't match.
</Tip>
