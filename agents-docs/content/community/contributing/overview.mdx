...
This automatically sets up SigNoz, OTEL Collector, Jaeger, and all related env vars. Restart `pnpm dev`, then open `http://localhost:3080` to view traces.
...
This automatically sets up Nango and wires the secret key to your `.env`. Restart `pnpm dev`, then create a credential in the Visual Builder (Credentials tab).
