...
5. **Environment export** — `env <name>` reads the state file and exports database URLs, Mailpit SMTP settings, app ports (`AGENTS_API_PORT`, `MANAGE_UI_PORT`), and API URLs (`INKEEP_AGENTS_API_URL`) so both backing services and the app layer use isolated ports

The backing services are the same as `pnpm setup-dev`: Doltgres (manage DB), Postgres (runtime DB), SpiceDB + its Postgres (authorization), and Mailpit (local email capture).
