...
- The **conversation trace timeline** shows a per-call cache-state badge (HIT / MISS / NOT-ATTEMPTED / NOT-SUPPORTED-BY-PROVIDER) next to each call's cost.
...
See [Prompt Caching](/guides/observability/prompt-caching) for the full cache-state taxonomy.
