...
<Image
  ...
  alt="SigNoz Traces Explorer showing inkeep-agents-api service traces with timestamp, service name, operation name, duration, HTTP method, and response status code columns"
...
- **Service Name**: The service that generated the span (e.g., `inkeep-agents-api`)
...
```
# Filter by service
service_name = "inkeep-agents-api"
...
- **Service**: The primary service (e.g., `inkeep-agents-api`)
...
Every LLM generation span includes prompt-cache telemetry. In the Manage UI:

- The **conversation trace timeline** shows a per-call cache-state badge (HIT / MISS-regression / MISS-expected / NOT-ATTEMPTED) next to each call's cost.
- The **cost dashboard** includes a **Cost by Cache Participation** breakdown alongside the existing Cost by Model, Agent, Provider, and Generation Type tables.

You can also query raw cache attributes directly in SigNoz using the span attributes emitted by the gateway cost middleware:

```
gen_ai.usage.cache_read.input_tokens > 0
gen_ai.usage.cache_creation.input_tokens > 0
cache.intent.marker_count > 0
```

See [Prompt Caching](/guides/observability/prompt-caching) for the full cache-state taxonomy and how to interpret MISS-regression signals.
