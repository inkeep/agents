...
If your Sub Agent gives weak or inconsistent results, you usually do not need a full rewrite.
Start by matching symptoms, apply one targeted fix, and test again.

## Start here

<Steps>
  <Step>
    Use the symptom picker to find the closest failure pattern.
  </Step>
  <Step>
    Copy one fix into your prompt.
  </Step>
  <Step>
    Re-run the same task 2-3 times.
  </Step>
  <Step>
    Keep the fix if behavior improves; remove it if it does not.
  </Step>
</Steps>
...
| Good local output, bad downstream impact | [Decision boundaries and risk](#decision-boundaries-and-risk) |
...
<Accordions>
  <Accordion title="The agent misses downstream effects">
    **What you see:** A local change seems fine but causes breakage elsewhere.

    **Why it happens:** The prompt does not require a pre-change blast-radius check.
  ...
  <Accordion title="The agent presents one path with overconfidence">
    **What you see:** One recommendation is presented even when multiple valid options exist.
