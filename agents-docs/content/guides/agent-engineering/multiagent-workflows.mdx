...
description: Design multiagent workflows with coordinator sub agents that delegate to specialists, aggregate results and drive the overall process forward.
icon: LuWorkflow
---
...
<Tip>Quick rule: if one task changes what the next task should do, use sequential delegation. If tasks can all start with the same context and only need to be combined at the end, use parallel delegation.</Tip>
...
## Constraining intermediate outputs

In multi-stage pipelines (for example `query → select → respond`), intermediate sub-agents should return structured data rather than narrate to the user. Use [output contracts](/typescript-sdk/structured-outputs/output-contract) to enforce this:

```typescript
const queryAgent = subAgent({
  id: 'query',
  name: 'Query Agent',
  prompt: 'Search and return results.',
...
The `allowText: false` setting removes the prose option from the model's response schema, so the sub-agent cannot narrate regardless of prompt drift.
