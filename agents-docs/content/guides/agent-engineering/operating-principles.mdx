...
These principles apply to any Sub Agent prompt you write — whether the agent is a router, specialist, coordinator, or standalone worker.
...
## 6. Make prompts standalone

Assume a first-time reader with zero context. Don't rely on the agent inferring from prior conversation or shared state.

For **specialist Sub Agents**:
- Don't reference other Sub Agents by name — write each as if it's the only agent that exists
- If a specialist needs workflow context, the coordinator passes it when delegating, not in the permanent prompt

For **coordinator Sub Agents** (the exception):
- Coordinators manage other Sub Agents by design, so they can reference Sub Agent names and routing rules

| Specialist prompt (avoid) | Specialist prompt (prefer) |
|---|---|
| "The coordinator will pass your findings to the implementer" | "Return findings in the output format below" |
