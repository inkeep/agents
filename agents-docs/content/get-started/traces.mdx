...
<Tip>
You can also access agent-specific traces directly from an agent's detail page by clicking the **View Traces** button in the toolbar. This opens the Traces view pre-filtered to show only traces for that agent.
</Tip>
