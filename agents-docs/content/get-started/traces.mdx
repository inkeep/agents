...
Conversations originating from the [Slack integration](/talk-to-your-agents/slack/configuration) are visually distinguished in the traces UI. Slack conversations display a `#` badge in the conversation list and a "Slack message received" label in the trace timeline, making it easy to identify Slack activity at a glance.
