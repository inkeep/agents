...
This automatically:
- Clones the `agents-optional-local-dev` companion repo (if not already present)
- Starts SigNoz, OTEL Collector, and Jaeger via Docker Compose
- Registers a SigNoz admin account and creates an API key
- Writes all required env vars (`SIGNOZ_URL`, `SIGNOZ_API_KEY`, `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`) to your `.env`

After setup completes, restart your dev server and traces will flow automatically:
...
Use `pnpm setup-dev:status` to check service health, `pnpm setup-dev:stop` to stop optional services, or `pnpm setup-dev:reset` to start fresh.
