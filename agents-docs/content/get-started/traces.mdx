...
icon: LuActivity
keywords: SigNoz, traces, live debugger, observability, OpenTelemetry, monitoring, distributed tracing, debugging, copy trace, export trace, JSON export, conversation transcript
---
...
When viewing a conversation trace, you can switch between two views:

- **Timeline view** — Shows the step-by-step execution flow with tool calls, agent transfers, and timing information. This is useful for debugging agent behavior and understanding the internal execution.
- **Conversation transcript view** — Displays the conversation in a familiar chat interface format, showing messages as they appeared to the end user. This is useful for reviewing the conversation flow from the user's perspective and sharing conversation examples with stakeholders.

Toggle between these views using the view selector at the top of the conversation detail panel.

<Snippet file="copy-trace.mdx" />

#### Time to first token (TTFT)

Each conversation records how quickly the agent started responding. The conversation detail view shows the **time to first token** — the time from request arrival to the first user-visible text token.

Three interaction-level timings are recorded as span attributes (in seconds) on the request span, and surfaced in the assistant message's span panel:

- `inkeep.agent.time_to_first_model_token` — first raw token from the model (model + network latency).
- `inkeep.agent.time_to_first_visible_token` — first user-visible **text** token (this is the headline "time to first token"). Omitted for turns that never produce text (for example, tool-only or artifact-only responses).
- `inkeep.agent.time_to_first_visible_part` — first user-visible output of **any** kind (text, a tool card, or an artifact). On a typical text-first turn this matches the visible-token timing; it diverges only when a tool card or artifact renders before any text.

The gap between the model timing and the visible-text timing reflects the framework's own parsing and buffering overhead. All three are queryable in SigNoz by `conversation.id` and `agent.id`.
