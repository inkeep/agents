...
<Tip>
  For programmatic monitoring, use the [Get Tool Status for Agent](/api-reference/agents#get-tool-status-for-agent) endpoint to retrieve MCP tool health across all sub-agents. It returns each tool's `status` (`healthy`, `unhealthy`, `unknown`, `needs_auth`, `unavailable`), `lastError`, and which `subAgentIds` use the tool. Use the optional `?status=` query parameter to filter by a specific status.
</Tip>
