...
2. Run `pnpm setup-dev` — this is idempotent and will start any stopped databases, apply pending migrations, and re-create the admin user
3. Run `pnpm dev` again

For the [quickstart template](/quick-start/start-development), the same applies: run `pnpm setup-dev` from the workspace root before `pnpm dev`.
...
- **API not running**: The auto-login calls `POST /api/auth/dev-session` on the API server. Make sure the API is running on `localhost:3002`.
