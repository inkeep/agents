...
- **MCP server creation hangs**:
    - For servers not using OAuth 2.1/PKCE, ensure you select **"No Authentication"** or an existing credential from the **Credential** dropdown
    - Leaving the Credential field blank on non-OAuth servers causes the creation process to hang indefinitely
