...
You can scaffold a new MCP server from our [templates repository](https://github.com/inkeep/agents-cookbook) using the [CLI](/typescript-sdk/cli-reference#inkeep-add).
```bash
inkeep add --mcp [server-name]
```

This will automatically scaffold a new MCP server in your Quick Start workspace. After customizing and deploying it, you can register it with your agents.
