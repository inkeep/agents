...
## Step 3: Check service logs in local development

When running `pnpm dev` from your [quickstart workspace](/quick-start/start-development), you will see an interactive terminal interface that allows you to inspect the logs of each running service. You can navigate between services using the up and down arrow keys.
...
To terminate the running services, click on the `Processes` box and press `q`.
