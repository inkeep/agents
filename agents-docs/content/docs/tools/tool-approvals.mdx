...
1. Agent calls tool → 2. Execution pauses → 3. Approval request emitted
                                                           ↓
6. Tool executes ← 5. Execution resumes ← 4. User approves/denies
```
...
export function ChatWithApprovals() {
  ...
  // Monitor messages for tool approval requests
  useEffect(() => {
    // Find the latest data operation message
    const latestDataOp = messages
      .filter((msg) => msg.data)
      .reverse()
      .find((msg) => {
        const data = msg.data;
        return (
          ...
          data.data?.details?.data?.needsApproval
        );
      });

    if (latestDataOp?.data) {
      const { toolName, input, toolCallId, conversationId } =
        latestDataOp.data.data.details.data;

      // Only show approval if we haven't already handled this tool call
      if (!approvalRequest || approvalRequest.toolCallId !== toolCallId) {
        setApprovalRequest({
          toolName,
          input,
          toolCallId,
  ...
  return (
    <div>
      ...
      <div>
        {messages
          .filter((msg) => msg.role !== "data") // Hide data operations from chat
          .map((message) => (
            <div key={message.id}>
      ...
      {approvalRequest && (
        <ApprovalDialog
          ...
          arguments={approvalRequest.input}
