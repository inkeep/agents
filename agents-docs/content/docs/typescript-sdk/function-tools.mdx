...
Function tools allow you to create custom JavaScript functions that agents can execute in secure, isolated sandboxes. Unlike [MCP servers](/typescript-sdk/tools-and-mcp-servers) that connect to external services, function tools run your own code directly within the agent framework.
...
Function tools run in secure Node.js sandboxes with the following characteristics:

### Sandbox Configuration

Function tools respect your project's sandbox configuration:

```typescript
// Project configuration (inkeep.config.ts)
export const project = project({
  // ... other config
  sandboxConfig: {
    provider: "local", // Execution provider
...
- **No file system write access** outside the sandbox
- **Network access** only via installed dependencies (like axios)
- **No access** to parent process or other sandboxes
