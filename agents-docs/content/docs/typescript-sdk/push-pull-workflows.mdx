...
graph TD
    ...
    M -->|Yes| N[Load environments/env.env.ts]
    M -->|No| O[Final Configuration]

    N --> P{Environment file exists?}
...
graph TD
    ...
    B --> C[TypeScript Files]
    C --> D[Exclude patterns]

    D --> E{File location}
    E -->|/node_modules/| F[Skip]
    E -->|/.git/| F
    E -->|/environments/| G[Environment Config]
    E -->|Other .ts files| H[Project Resource]

    H --> I[Parse TypeScript]
    I --> J[Extract exports]
    J --> K{Export type}

    K -->|project()| L[Project Definition]
    K -->|agentGraph()| M[Graph Definition]
    K -->|agent()| N[Agent Definition]
    K -->|tool()| O[Tool Definition]
    K -->|dataComponent()| P[Data Component]
    K -->|Other| Q[Generic Resource]

    L --> R[Compile Resources]
    M --> R
    N --> R
    O --> R
    P --> R
    G --> S[Load Environment]
