...
// 1. Define request schema for headers validation. 
...
When you define a `requestContextSchema` using Zod object schemas, the framework automatically applies `.loose()` to make validation more permissive:
...
This means your request context can include additional properties beyond what's defined in your schema without causing validation errors.

**Note:** Only `ZodObject` schemas get automatic `.loose()` application. Other schema types (strings, arrays, unions, etc.) remain unchanged.
