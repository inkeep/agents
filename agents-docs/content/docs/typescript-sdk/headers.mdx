...
const userFetcher = fetchDefinition({
  ...
  fetchConfig: {
    ...
    method: "GET",
    headers: {
      Authorization: `Bearer ${personalGraphHeaders.toTemplate('auth_token')}`,
    },
...
const personalAssistant = agent({
  ...
  prompt: `You are a helpful assistant for ${personalGraphContext.toTemplate('user.name')} from ${personalGraphHeaders.toTemplate('org_name')}.

  User ID: ${personalGraphHeaders.toTemplate('user_id')}
...
const userDataFetcher = fetchDefinition({
  ...
  name: "User Data",
  fetchConfig: {
    url: `https://api.example.com/users/${personalGraphHeaders.toTemplate('user_id')}/profile`,
    headers: {
      Authorization: `Bearer ${personalGraphHeaders.toTemplate('auth_token')}`,
      "X-Organization": personalGraphHeaders.toTemplate('org_name')
    },
    body: {
      includePreferences: true,
      userId: personalGraphHeaders.toTemplate('user_id')
    }
  },
  responseSchema: z.object({
    name: z.string(),
...
const externalAgent = externalAgent({
  ...
  headers: {
    Authorization: `Bearer ${personalGraphHeaders.toTemplate('auth_token')}`,
    "X-User-Context": personalGraphHeaders.toTemplate('user_id'),
    "X-Org": personalGraphHeaders.toTemplate('org_name')
  }
});

// Configure context for your graph with your headers schema.
const personalGraphContext = contextConfig({
  headers: personalGraphHeaders,
});
```
...
- **Secure sensitive data**: For long-lived secrets, use the [Credentials](/typescript-sdk/credentials) system instead of headers
