...
// Create a context fetcher that uses request values with type-safe templating
const userFetcher = fetchDefinition({
  id: "user-info",
  name: "User Information",
  trigger: "initialization",
  fetchConfig: {
    url: `https://api.example.com/users/${requestHeaders.toTemplate('user_id')}`,
    method: "GET",
    headers: {
      Authorization: `Bearer ${requestHeaders.toTemplate('auth_token')}`,
    },
...
const userContext = contextConfig({
  ...
  headers: requestHeaders, // Use the builder for consistency
  contextVariables: {
...
const personalAssistant = agent({
  ...
  prompt: `You are a helpful assistant for ${userContext.toTemplate('userName')} from ${userContext.toTemplate('headers.org_name')}.

  User ID: ${userContext.toTemplate('headers.user_id')}
...
Request context values can be used in your agent prompts and fetch definitions using JSONPath template syntax `{{headers.field_name}}`.
...
const userDataFetcher = fetchDefinition({
  ...
  name: "User Data",
  fetchConfig: {
    url: `https://api.example.com/users/${requestHeaders.toTemplate('user_id')}/profile`,
    headers: {
      Authorization: `Bearer ${requestHeaders.toTemplate('auth_token')}`,
      "X-Organization": requestHeaders.toTemplate('org_name')
    },
    body: {
      includePreferences: true,
      userId: requestHeaders.toTemplate('user_id')
    }
  },
...
```typescript
// Create context config with both request context and fetchers
const userContext = contextConfig({
  id: "user-context",
  name: "User Context",
...
const assistantAgent = agent({
  prompt: `You are an assistant for ${userContext.toTemplate('userName')} from ${userContext.toTemplate('headers.org_name')}.

  User context:
  - ID: ${userContext.toTemplate('headers.user_id')}
  - Organization: ${userContext.toTemplate('headers.org_name')}
...
Configure external agents or MCP servers with dynamic headers using request context:
...
const externalAgent = externalAgent({
  ...
  headers: {
    Authorization: `Bearer ${requestHeaders.toTemplate('auth_token')}`,
    "X-User-Context": requestHeaders.toTemplate('user_id'),
    "X-Org": requestHeaders.toTemplate('org_name')
  }
});
```
