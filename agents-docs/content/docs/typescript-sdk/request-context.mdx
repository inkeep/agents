...
const userFetcher = fetchDefinition({
  ...
  fetchConfig: {
    ...
    method: "GET",
    headers: {
      Authorization: `Bearer ${requestContext.toTemplate('auth_token')}`,
    },
...
const userContext = contextConfig({
  ...
  requestContextSchema: requestContext, // Use the builder for consistency
  contextVariables: {
...
const personalAssistant = agent({
  ...
  prompt: `You are a helpful assistant for ${userContext.toTemplate('userName')} from ${userContext.toTemplate('requestContext.org_name')}.
  
  User ID: ${userContext.toTemplate('requestContext.user_id')}
...
const userDataFetcher = fetchDefinition({
  ...
  name: "User Data",
  fetchConfig: {
    url: `https://api.example.com/users/${requestContext.toTemplate('user_id')}/profile`,
    headers: {
      Authorization: `Bearer ${requestContext.toTemplate('auth_token')}`,
      "X-Organization": requestContext.toTemplate('org_name')
    },
    body: {
      includePreferences: true,
      userId: requestContext.toTemplate('user_id')
    }
  },
...
const assistantAgent = agent({
  prompt: `You are an assistant for ${userContext.toTemplate('userName')} from ${userContext.toTemplate('requestContext.org_name')}.
  
  User context:
  - ID: ${userContext.toTemplate('requestContext.user_id')}
  - Organization: ${userContext.toTemplate('requestContext.org_name')}
...
const externalAgent = externalAgent({
  ...
  headers: {
    Authorization: `Bearer ${requestContext.toTemplate('auth_token')}`,
    "X-User-Context": requestContext.toTemplate('user_id'),
    "X-Org": requestContext.toTemplate('org_name')
  }
});
```
