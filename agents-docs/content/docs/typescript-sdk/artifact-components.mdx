...
1. **With Schema**: Define specific fields and mark preview fields with `isPreview: true`
2. **No Schema**: Save the entire tool result as-is (no field filtering)

When using a schema:
- **Preview fields** (`isPreview: true`): Shown to agents automatically and streamed in real-time  
  ...
  artifactComponents: [
    {
      ...
      props: {
        ...
        properties: {
          ...
          url: { type: "string", isPreview: true },
          content: { type: "string" },
        },
      },
    },
