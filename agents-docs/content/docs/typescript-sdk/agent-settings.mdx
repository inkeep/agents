...
For accessing models not directly supported, you can use:
...
### Provider Options (Vercel AI SDK v5)

Each model settings can include `providerOptions` with both generic and provider-specific parameters:

#### Generic Parameters (All Providers)
These parameters work across all providers and go directly in `providerOptions`:

- `maxOutputTokens`: Maximum number of tokens to generate (**Note:** AI SDK v5 uses `maxOutputTokens`, not `maxTokens`)
- `temperature`: Controls randomness of output (0.0-1.0, lower is more deterministic)
- `topP`: Nucleus sampling - consider tokens with top cumulative probability (0.0-1.0)
- `topK`: Limits token sampling to top K options (integer)
- `frequencyPenalty`: Reduces repetition of words used frequently (-2.0 to 2.0)
- `presencePenalty`: Encourages model to explore new topics (-2.0 to 2.0)
- `stopSequences`: Array of sequences that stop generation (e.g., `["\n\n", "END"]`)
- `seed`: Random seed for deterministic output (integer)
- `maxDuration`: Request timeout in seconds (**Note:** seconds not milliseconds, e.g., `30` for 30 seconds)
- `maxRetries`: Maximum retry attempts on failure (default: 2)

#### Provider-Specific Parameters
These parameters must be wrapped in a provider-specific object (e.g., `anthropic: {}`, `openai: {}`, `google: {}`):
...
models: {
  base: {
    ...
    providerOptions: {
      ...
      // Provider-specific parameters (wrapped)
      anthropic: {
        thinking: {
          type: 'enabled',
          budgetTokens: 8000
