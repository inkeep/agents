...
- Managed through Nango (self-hosted or cloud). Follow steps to [setup Nango](/quick-start/credentials).
...
// Bearer authentication
retrievalParams: {
  "key": "<id-given-to-bearer-api-key>" // where <id-given-to-bearer-api-key> is the id given to the bearer api key created in through the Visual Builder
}
```
