...
- `--config-file-path <path>` - Path to configuration file (alternative to --tenant-id/--api-url)
...
inkeep chat --api-url http://localhost:3003
```
...
# List graphs with custom API
inkeep list-graphs --api-url http://localhost:3002 --tenant-id my-tenant
```
...
inkeep config set projectId my-project-id
```
...
```typescript
import { defineConfig } from "@inkeep/agents-sdk";
...
Resolution order varies slightly by command:

1. Command-line flags (highest)
2. `inkeep.config.ts` values
3. Environment variables (when applicable to the command)
...
- The CLI will prompt you to create the project if it doesn't exist during push
