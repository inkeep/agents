...
Install the Inkeep CLI globally using npm or pnpm:
...
sudo npm install -g @inkeep/agents-cli
...
Pull a template project from the [Inkeep Agents Cookbook](https://github.com/inkeep/agents-cookbook/tree/main/templates).
...
```bash
inkeep add weather-graph
inkeep add weather-graph --target-path ./my-project-directory
```
...
### Working with a Self-Hosted Server

When working with a self-hosted Inkeep server (for development or on-premise deployment):
...
2. Configure your API URL to point to your server:
