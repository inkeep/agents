...
2. **Project Structure**: Expects this directory layout:
   ```
   my-project/
   ├── inkeep.config.ts          # Configuration file
...
- Loads configuration from `inkeep.config.ts`
- Applies environment-specific settings if `--env` is specified
- Compiles all project resources defined in your `index.ts`
- Validates agent relationships and tool configurations
- Deploys to the management API
- Prints deployment summary with resource counts
...
- `--no-cache` - Force fresh data fetch from server (bypasses caching)
