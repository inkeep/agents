...
const qaAgent = agent({
  ...
  prompt: "You are a QA agent that can answer questions about the company knowledge base.",
...
Add custom icons to your MCP tools using the `imageUrl` option. Supports both URLs and base64 data URLs.

```typescript
import { mcpServer, mcpTool } from "@inkeep/agents-sdk";
...
```typescript
import { mcpServer, mcpTool, agent } from "@inkeep/agents-sdk";

// Create tools with clear naming and custom icons
const weatherTool = mcpServer({
  name: "weather_service",
  description: "Get real-time weather data and forecasts for any location",
  serverUrl: "https://api.weather.com/mcp",
  imageUrl: "https://i.pinimg.com/originals/d5/3b/01/d53b014d86a6b6761bf649a0ed813c2b.png", // Custom weather icon
  activeTools: ["get_current_weather", "get_forecast"], // Only expose needed capabilities
});
...
const productTool = mcpTool({
  ...
  imageUrl: "https://cdn.example.com/icons/product.svg", // Custom product icon
