...
Ensure Docker Desktop (or Docker daemon) is running before running the setup script as it will start a local database container.

```bash
pnpm setup-dev
```

Or set your DATABASE_URL environment variable to a cloud database URL and run:
