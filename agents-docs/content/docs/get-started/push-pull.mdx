...
Make some changes, like changing a prompt, and let's walk through `inkeep pull`. The `inkeep pull` command uses AI to sync your TypeScript files to the state of your Visual Builder.

<Note>
**Prerequisites:** The `inkeep pull` command uses AI to generate TypeScript files from your project configuration. You must have an API key from one of the supported LLM providers:
...
**Google** (Gemini models with thinking mode):
```bash
export GOOGLE_API_KEY=your_api_key_here
```
Get your API key from [Google AI Studio](https://ai.google.dev/).

The pull command will automatically detect which provider to use based on which API key is available. If multiple keys are present, it prioritizes Anthropic → OpenAI → Google.

Or add your preferred key to your `.env` file:
```bash
# Choose one:
