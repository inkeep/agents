---
title: Agents
description: Agents define the relationships between sub-agents and tools.
icon: "LuSpline"
---
...
Tools are used to perform actions in the agent. You can create MCP tools in the Visual Builder, see the [MCP Servers](/visual-builder/tools/mcp-servers) page for more information.

Once you have a tool, add the tool to the agent by dragging and dropping the MCP tool box into the agent.
...
Sub-agents are specialized sub-agents that handle specific tasks within your main sub-agent's workflow. They allow you to delegate focused responsibilities to separate sub-agents with their own prompt and capabilities.

To add a sub-agent:

1. Drag and drop a sub-agent box from the toolbar into the agent
