...
Sub Agent: {
  models: {
    ...
    // base: INHERITED from Agent (gpt-4.1)
    // structuredOutput: INHERITED from project (gpt-4.1-mini)
  }
}
```
...
- **base**: `gpt-4.1-2025-04-14` (from Sub Agent)
- **summarizer**: `gpt-4.1-mini-2025-04-14` (inherited from project - NO fallback to Sub Agent's base)
- **structuredOutput**: Falls back to Sub Agent's base `gpt-4.1-2025-04-14` (no inheritance available, so fallback occurs)

### StopWhen Inheritance

Different cascade rules apply based on the control scope:

- **`transferCountIs`**: Controls agent-level behavior → **Project** → **Agent** only
- **`stepCountIs`**: Controls individual Sub Agent behavior → **Project** → **Agent** → **Sub Agent**
