...
Function tools are created within a graph and can be used by multiple agents in that graph.

### In a Graph

1. **Add a Function Tool Node**: In the graph editor, drag a "Function Tool" node from the node palette onto the canvas
