...
With Inkeep, there's a few ways to do this:
...
- **Artifacts**: An Agent save information from a **tool call result** as an artifact in order to make it available to others. For example, an Agent that did a web search can save the contents of a webpage it looked at as an artifact. Once saved, an Agent can cite or reference artifacts in its response, and other agents or users can fetch the full artifacts if they'd like.
- **Status Updates**: Real-time progress updates that can be plain text or Structure Outputs that can be used to keep users informed about what the agent is doing during longer operations. 
