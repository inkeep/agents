...
Under the hood, an Agent is made up of one or more **Sub Agents** that work together to complete a task and respond to the user. 
...
In Inkeep, tools are added to Sub Agents as **MCP Servers**. Many services provide out-of-the-box MCP Servers, but you can also create your own and register them and their associated **Credentials** on Inkeep for Agents to use.

Typically, you want a Sub Agent to handle narrow, well-defined tasks. Keeping the scope narrow allows you to optimize your **prompt** (instructions) and keep the tools available to a Sub Agent relevant to a specific scenario or task. 

As a general rule of thumb, only add up to 5-7 tools to a Sub Agent.

### Sub Agent relationships

When what you're trying to accomplish gets complex, it can be useful to break up your logic by creating Sub Agents that are specialized in specific parts of your task or workflow. This is often referred to as a "Multi-Agent" system.
...
An Agent's execution stays in this loop until one of the Sub Agents responds to the user with a final result.

<Note>
Sub Agents in Inkeep are designed to automatically respond to the user as a single, cohesive unit.
</Note>
