...
description: Use the A2A JSON-RPC protocol to send messages to your agent and receive results, with optional streaming.
icon: "LuNetwork"
---

The A2A (Agent-to-Agent) endpoint lets external agents/services talk to your agent using a JSON-RPC 2.0 protocol compatible with Google's A2A schema.

<Snippet file="auth/RunApiAuthModes.mdx" />

## Endpoints

- **Agent card discovery (agent or sub-agent-level):** `GET /agents/.well-known/agent.json`
- **A2A protocol (agent or sub-agent-level):** `POST /agents/a2a`

Notes:
- If you supply `x-inkeep-sub-agent-id` in headers, requests target that specific agent. This is supported in development (or when using the bypass secret). With production API keys, requests always use the graph’s default agent. API keys bound to a specific agent are not yet supported.
...
- **Graph-level:** `GET /agents/.well-known/agent.json` (uses agent default sub-agent)
