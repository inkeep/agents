...
Artifact components use a single unified schema called `props` with `isPreview` indicators.

### Unified props schema

Fields marked with `isPreview: true` appear in summary views, while all fields are stored in the database:
