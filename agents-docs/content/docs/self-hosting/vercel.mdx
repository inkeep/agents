...
<Note>
  **Environment Variables**: Function tools can access environment variables
  from the host. The framework automatically detects `process.env` references in
  your code and injects the corresponding values into the sandbox via a `.env`
  file.
</Note>
