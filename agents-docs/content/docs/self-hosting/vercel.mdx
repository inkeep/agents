...
| `PUBLIC_INKEEP_AGENTS_MANAGE_API_URL`        | Your Vercel deployment URL for Manage API                                                                                                                                                      |
| `PUBLIC_INKEEP_AGENTS_RUN_API_URL`           | Your Vercel deployment URL for Manage API                                                                                                                                                      |
...
When deploying to serverless environments like Vercel, you can configure [function tools](/typescript-sdk/tools/function-tools) to execute in Vercel Sandbox MicroVMs instead of native Node.js processes. This is **required** for serverless platforms since child process spawning is restricted.

### Why Use Vercel Sandbox?

**Native execution limitations in serverless:**

- Serverless platforms (Vercel, AWS Lambda, Cloudflare Workers) restrict child process spawning
- Function tools using the native provider will fail or timeout in these environments
...
- **Vercel Sandbox** - Required for serverless platforms (Vercel, AWS Lambda, etc.)

**Vercel Sandbox benefits:**

- ✅ Designed specifically for serverless environments
- ✅ Isolated Linux MicroVMs with full Node.js support
- ✅ Configurable CPU and memory resources
- ✅ Reliable execution without child process restrictions
...
#### Step 2: Add Environment Variables to Run API

Add these environment variables to your **Run API** Vercel project:
...
<Warning>
  Never commit these credentials to your repository. Always use environment
  variables.
</Warning>

#### Step 3: Configure Sandbox in Your Application

Update your Run API to use Vercel Sandbox:

```typescript filename="apps/run-api/sandbox.ts"
import { SandboxConfig } from "@inkeep/agents-run-api";
...
To save costs, you can use native for local development and Vercel Sandbox for serverless production:

```typescript
const isProduction = process.env.ENVIRONMENT === "production";

const app = createExecutionApp({
  sandboxConfig: isProduction
    ? {
        provider: "vercel",
        runtime: "node22",
        timeout: 60000,
...
- ✅ Develop and test locally without Vercel credentials
- ✅ Use Vercel Sandbox only in serverless production
- ✅ Keep development fast with native execution
- ✅ Use native provider in traditional cloud environments (VMs, Docker, K8s)

### Troubleshooting

**"Failed to refresh OIDC token" error:**

- This occurs when using not in vercel environment and you're not passing in a Vercel access token
- Solution: Use a Vercel access token from [vercel.com/account/tokens](https://vercel.com/account/tokens)
...
- [Function Tools (TypeScript SDK)](/typescript-sdk/tools/function-tools)
- [Function Tools (Visual Builder)](/visual-builder/tools/function-tools)
