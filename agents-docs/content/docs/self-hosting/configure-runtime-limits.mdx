...
**2. Runtime Execution Constants** (`packages/agents-core/src/constants/execution-limits-shared/defaults.ts`)
...
All runtime configuration variables are defined as TypeScript constants in the `@inkeep/agents-core` package. To explore available overrides and their detailed documentation:
...
1. **View the constant files** (source of truth):
   ...
   - Runtime execution constants: `packages/agents-core/src/constants/execution-limits-shared/defaults.ts`
...
**Runtime Execution Constants** (infrastructure-level limits):
...
**Additional Runtime Settings** (defined elsewhere in the codebase):
