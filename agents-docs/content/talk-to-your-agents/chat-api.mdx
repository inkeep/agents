...
- **`content`** — Can be a string or an array of content parts for multi-part content (text, images)
...
<Note>
  Images are validated server-side: MIME types are verified against actual content via byte-sniffing, size limits are enforced, and external URLs must use HTTP/HTTPS with no embedded credentials. DNS checks block requests to private/internal IP addresses.
</Note>
