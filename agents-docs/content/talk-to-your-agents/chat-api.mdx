...
## File inputs (images, PDFs, text documents, and office documents)
...
- **Files (`file`)**: support PDFs, inline text document types, and inline office document types:
  - **Text documents:** `text/plain`, `text/markdown`, `text/html`, `text/csv`, `text/x-log`, `application/json`
  - **Office documents:** `application/vnd.openxmlformats-officedocument.wordprocessingml.document` (`.docx`), `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` (`.xlsx`), `application/vnd.openxmlformats-officedocument.presentationml.presentation` (`.pptx`), `application/vnd.oasis.opendocument.text` (`.odt`), `application/vnd.oasis.opendocument.spreadsheet` (`.ods`), `application/vnd.oasis.opendocument.presentation` (`.odp`), `application/vnd.apple.pages` (`.pages`), `application/vnd.apple.numbers` (`.numbers`), `application/vnd.apple.keynote` (`.key`)
...
- **Office document file inputs**: support inline base64 `data:` payloads only. Remote URLs are not accepted for office document MIME types.

<Note>
  **Provider compatibility for office documents:** Office document file parts are passed through natively to OpenAI and Azure OpenAI models. For other providers (Anthropic, Google Gemini, etc.), office document parts are automatically stripped from the request and the model receives a note explaining that the attachment was unavailable.
</Note>
...
- **Maximum size**:
  - Images, PDFs, and office documents: each inline file payload is limited to **10 MB**
  - Text documents (`text/plain`, `text/markdown`, `text/html`, `text/csv`, `text/x-log`, `application/json`): each inline file payload is limited to **256 KB**
...
- **Content validation**:
  ...
  - Office documents must begin with valid ZIP magic bytes (`PK`)
...
- **Remote text document URLs**: are not supported. Use inline base64 `data:text/...;base64,...` or `data:application/json;base64,...` payloads instead.
- **Remote office document URLs**: are not supported. Use inline base64 `data:` payloads instead.
