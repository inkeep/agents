...
If the SSE stream disconnects before the client receives the `tool-approval-request` event, the approval request is still persisted server-side. Your client can poll `GET /run/v1/conversations/{conversationId}/pending-approvals` to recover pending approval state on page load or after a reconnection. See [Tool approvals](/typescript-sdk/tools/tool-approvals) for implementation details.
