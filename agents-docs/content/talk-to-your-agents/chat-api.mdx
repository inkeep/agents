...
Messages can include images alongside text using the `image_url` content type:
...
**Supported formats:** PNG, JPEG, WebP (GIF is not supported by all providers)

<Note>
  Images are processed for the current request but are not persisted to conversation history.
</Note>

**Field Notes:**

- **`messages`** — Must include at least one `user` message
- **`content`** — Can be a string or an array of content items (text and images)
- **`image_url.detail`** — Optional; controls image processing detail (`"auto"`, `"low"`, or `"high"`)
