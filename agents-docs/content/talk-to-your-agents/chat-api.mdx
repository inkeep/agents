...
### Recovering pending approvals

If the SSE stream disconnects before the client receives a `tool-approval-request` event, or if the user refreshes the page while an approval is pending, you can poll for pending approvals:

**Endpoint:** `GET /run/v1/conversations/{conversationId}/pending-approvals`
...
When `hasPending` is `false`, the `approval` field is omitted. Use this endpoint on page load or after SSE reconnection to restore the approval UI state.

<Note>
  This endpoint only returns pending approvals for agents running in [Durable mode](/visual-builder/execution-modes#durable-mode). In Classic mode, pending approvals are request-scoped and not recoverable after stream disconnection.
</Note>
