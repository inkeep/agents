...
- **`messages[].id`** — Optional; client-supplied message ID (AI SDK format). Alphanumeric plus `_` and `-`, max 64 characters. Takes precedence over `messageId` in the request body.
...
- **`messageId`** — Optional; client-supplied ID for the user message. Same format constraints as `messages[].id`. When provided, this ID is persisted as the `messages.id` row, enabling correlation with analytics events fired client-side (e.g. `user_message_submitted`). The server reads `messages[].id` first, falls back to `messageId`, then generates one if neither is provided.
...
- **`409`** — Message with the supplied `messageId` already exists
