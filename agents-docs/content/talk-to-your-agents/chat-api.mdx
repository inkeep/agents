...
- **Files (`file`)**: support PDFs, inline UTF-8 text/code/config document formats (plain text, Markdown, HTML, CSV, logs, JSON, JavaScript, XML, YAML (`.yaml` and `.yml`), CSS, shell scripts, source files, templates, subtitles, calendars, vCards, and RFC 822 email messages), and ZIP-based office documents (`.docx`, `.xlsx`, `.pptx`, `.odt`, `.ods`, `.odp`, `.pages`, `.numbers`, `.key`)
...
- **Office document file inputs**: support inline base64 `data:` payloads only. Model-aware stripping is applied automatically for providers that do not support these inline document parts.
