...
- **`id`** (per message) — Optional client-supplied message ID. When provided, this value is persisted as `messages.id` and can be used to correlate client-side analytics events with the server-persisted message row via the [Events API](/talk-to-your-agents/events-api) `messageId` field.
