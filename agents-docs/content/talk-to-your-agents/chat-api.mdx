...
- **`content`** — Can be a string, or use `parts` array for multi-part content including images
- **`parts`** — Array of content parts supporting `text` and `image` types
- **`conversationId`** — Optional; server generates one if omitted


### Multi-part Content with Images

To include images alongside text, use the `parts` array:
  ...
  "messages": [
    {
      ...
      "parts": [
        ...
        { "type": "image", "text": "https://example.com/photo.png" }
      ]
    }
  ],
  "conversationId": "optional-conversation-id"
...
- PNG, JPEG, JPG, WebP
- GIF is not supported by all model providers
