...
- **Files (`file`)**: currently support **PDF only**, and only as base64 `data:` URIs
- **External PDF URLs are rejected**: pass PDFs inline as `data:application/pdf;base64,...`
...
- **Vercel `parts` file typing**: include at least one of `mediaType` or `mimeType`
  - If both are present, `mediaType` is preferred
- **Content validation**:
  - Images are validated against supported image bytes
  - PDFs must start with the `%PDF-` signature bytes

<Note>
  This is intentionally asymmetric: `image_url` accepts both remote URLs and data URIs, while
  `file.file_data` accepts PDF data URIs only.
</Note>
