...
```json
{
  ...
  "userProperties": { "userId": "user-123", "plan": "pro" },
  "properties": { "source": "chat-widget", "pageUrl": "https://example.com/docs" }
}
```
...
- **`userProperties`** — Optional free-form JSON object for end-user attribution. Stored on the conversation and message records. See the [Events API](/talk-to-your-agents/events#user-attribution) for recommended field conventions.
- **`properties`** — Optional free-form JSON object for per-turn context (e.g. page URL, referrer). Stored on the conversation record.
