...
The `sendMessage` function supports sending images alongside text. Images can be provided as HTTP URLs or base64 data URIs.
...
<Note>
  Supported image formats: PNG, JPEG, and WebP. GIF is not supported by all providers.
</Note>
