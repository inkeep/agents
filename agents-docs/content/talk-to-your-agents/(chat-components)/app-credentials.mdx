...
You can also set this via the API by updating the app with `PATCH /manage/tenants/{tenantId}/projects/{projectId}/apps/{id}` and including `{ "config": { "type": "web_client", "webClient": { "allowAnonymous": false } } }` in the body.
