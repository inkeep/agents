...
Triggers simply put are HTTP URLs you create that you can send POST requests to, to invoke the agent. Once you create the webhook URL in the dashboard, you can hook it up to external services like GitHub, Slack, Zendesk, or Stripe to trigger your agent. An example of when a webhook URL comes in handy for example in Zendesk, you can go into Zendesk's configuration settings and input the webhook URL you created in the Inkeep dashboard to trigger your agent when a new ticket is created or updated. The same can be applied to other services like GitHub, PagerDuty, or Stripe.
