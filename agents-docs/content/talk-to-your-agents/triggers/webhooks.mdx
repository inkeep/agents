...
When `runAsUserId` is set, the system also resolves the user's profile timezone and uses it to populate the `x-inkeep-client-timezone` header during execution. This allows agents to receive proper timezone context for user-scoped triggers, enabling time-aware responses even for automated webhook invocations.
