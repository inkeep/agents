...
<Note>
For best results, pair report presets with an agent that uses the **report starter template** (available in the agent metadata editor). The starter prompt steers the agent toward executive summaries, chart artifacts, and conditional Slack/Jira delivery.
</Note>
