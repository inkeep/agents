...
If the app is uninstalled directly from Slack (via **Settings → Manage Apps**), Inkeep automatically detects the removal and cleans up all associated data.

In both cases, uninstalling removes the bot from your workspace, deletes all user link mappings, and cleans up channel configurations.
