...
<Note>
Only one Slack workspace can be connected per Inkeep tenant. Re-authorizing the same workspace (e.g. to refresh tokens) is supported, but installing a different workspace requires uninstalling the existing one first.
</Note>
...
The first time they interact with the bot, they'll be prompted to link their Slack account. They can also link proactively:
