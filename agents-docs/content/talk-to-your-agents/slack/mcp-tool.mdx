...
|-----------|------|----------|-------------|
| `channel` | string | Yes | Channel ID (e.g., `C1234567890`) or channel name with `#` prefix (e.g., `#general`). |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `user` | string | Yes | User ID or user mention (e.g., `U1234567890` or `@username`). |
| `text` | string | Yes | Message text. Supports [Slack mrkdwn formatting](https://api.slack.com/reference/surfaces/formatting). |
| `thread_ts` | string | No | Thread timestamp to reply in a thread. If omitted, posts as a new message. |

The agent knows how to send DMs and mention users out of the box — no additional prompting or configuration is required.
