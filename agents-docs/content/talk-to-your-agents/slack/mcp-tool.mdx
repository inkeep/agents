---
title: Slack MCP Tool
sidebarTitle: MCP Tool
description: Give your agents the ability to post messages to Slack channels and DMs via the MCP protocol.
---
...
<Steps>
  ...
  <Step>
    ...
    | **DM enabled** | Whether the agent can send direct messages. Disabled by default. |
...
The Slack MCP server exposes the following tool:
...
|-----------|------|----------|-------------|
| `channel` | string | Yes | Channel ID (e.g., `C1234567890`), channel name with `#` prefix (e.g., `#general`), or DM channel ID (starts with `D`). |
| `text` | string | Yes | Message text. Supports [Slack mrkdwn formatting](https://api.slack.com/reference/surfaces/formatting). |
| `thread_ts` | string | No | Thread timestamp to reply in a thread. If omitted, posts as a new message. |

When a channel name is provided (e.g., `#general`), it is automatically resolved to the channel ID via the Slack API.
...
2. In the **Slack Channel Access** section, click **Edit**
