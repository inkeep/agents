...
- **Public responses** — All agent responses are visible to everyone. `@Inkeep` replies in a thread, `/inkeep` posts at the channel root, and message shortcuts reply in a thread on the target message
- **Direct messages** — Mention `@Inkeep` or use `/inkeep` commands in your DMs for private conversations using your workspace default agent
...
4. The response appears publicly — as a threaded reply (`@Inkeep`, message shortcuts, DMs) or at the channel root (`/inkeep`)
