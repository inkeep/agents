...
Private responses are visible **only to you** — they don't appear in the channel for other users.

<Note>
Private responses use Slack's **ephemeral messages**. They are visible only to you and do not persist across page reloads, between desktop and mobile apps, or across sessions.
</Note>
...
Each follow-up appears as a new private message showing your question and the agent's response.
