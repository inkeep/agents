...
<Note>
The grant access setting applies per channel. Workspace defaults always grant access to members — the toggle is only available for channel-level overrides.
</Note>
