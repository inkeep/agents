...
Message `parts` can include text and file content. File parts support images via URL or base64-encoded data.
...
<Note>
  Images are processed by the agent but not persisted to conversation history.
</Note>
