...
- **Via API (Vercel format)**: An API that streams responses over server-side events (SSE). Use from any language/runtime, including the Vercel's `useChat` and AI Element primatives for custom UIs. See [API (Vercel format)](/talk-to-your-agents/api).
