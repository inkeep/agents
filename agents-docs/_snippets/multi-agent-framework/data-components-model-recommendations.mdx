## Model Recommendations

<Warning>
**Data components require high-capability models for best results.**

Configure the base model in your agent settings with one of these recommended models:

| Provider | Recommended Models |
|----------|-------------------|
| **Anthropic** | `claude-sonnet-4+`, `claude-opus-4+` |
| **OpenAI** | `gpt-4.1`, `gpt-5.1`, `gpt-5.2` |
| **Google** | `gemini-3.0-pro` |

**Not recommended:** Lower-tier models (`gpt-4.1-mini`, `gpt-4.1-nano`, `gemini-2.5-flash`, `gemini-2.5-flash-lite`), or custom models.

Data components use the base model for structured output generation, which requires frontier-level reasoning capabilities.
</Warning>
