...
function parseFileForComponents(
  ...
  try {
    ...
    const exportedIdPattern =
      /export\s+const\s+(\w+)\s*=\s*(\w+)\s*\(\s*\{[^}]*?id:\s*['"`]([^'"`]+)['"`]/gs;
    ...
    const exportedTypePattern =
      /export\s+const\s+(\w+)\s*=\s*(\w+)\s*\(\s*\{[^}]*?type:\s*['"`]([^'"`]+)['"`]/gs;
    ...
    //   }
    // });
    const envCredentialsPattern =
      /registerEnvironmentSettings\s*\(\s*\{[^}]*?credentials:\s*\{([^}]+?)\}/gs;
    let envMatch;
