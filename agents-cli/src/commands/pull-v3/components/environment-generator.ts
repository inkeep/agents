  ...
  for (const [key, credentialData] of Object.entries(credentials) as [string, any][]) {
    ...
    if (credentialData.type) {
      if (typeof credentialData.type === 'string') {
        ...
        const enumValue =
          credentialData.type === 'memory'
            ? 'CredentialStoreType.memory'
            : credentialData.type === 'env'
              ? 'CredentialStoreType.env'
              : credentialData.type === 'keychain'
    ...
    if (credentialData.credentialStoreId) {
      lines.push(
        `${nestedIndent}credentialStoreId: ${formatString(credentialData.credentialStoreId, q)},`
      );
    }
    ...
    if (credentialData.retrievalParams) {
      ...
      for (const [paramKey, paramValue] of Object.entries(credentialData.retrievalParams) as [
        string,
        any,
      ][]) {
...
export function generateEnvironmentSettingsImports(
  ...
  const needsCredentialStoreType =
    environmentData.credentials &&
    Object.values(environmentData.credentials).some(
      (cred: any) =>
