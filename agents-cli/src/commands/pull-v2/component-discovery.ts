...
export interface ComponentLocation {
  componentType: 'agent' | 'tool' | 'dataComponent' | 'artifactComponent' | 'statusComponent' | 'environment' | 'project';
  componentId: string;
  filePath: string;
  exportName: string;
  lineNumber?: number;
 ...
 */
export async function discoverComponentLocations(projectDir: string): Promise<Map<string, ComponentLocation>> {
...
function parseComponentsInFile(content: string, filePath: string): ComponentLocation[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const artifactComponentMatch = line.match(/export\s+const\s+(\w+)\s*=\s*artifactComponent\s*\(/);
    ...
    const environmentMatch = line.match(/export\s+const\s+(\w+)\s*=\s*registerEnvironmentSettings\s*\(/);
    if (environmentMatch) {
      const exportName = environmentMatch[1];
      // For environments, the ID is typically the export name or filename
      const id = exportName.replace(/([A-Z])/g, '-$1').toLowerCase().replace(/^-/, '');
      components.push({
