...
function processObject(obj: any, tracker: PlaceholderTracker, path: string = '', context?: { fileType?: string }): any {
  if (typeof obj === 'string') {
    // For long strings, ALWAYS replace the entire string with ONE placeholder
    // Don't fragment strings that contain template literals - keep them intact
  ...
  if (isJsonSchemaPath(path, context)) {
    try {
      // Validate that obj is a proper JSON schema before conversion
      if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
        console.warn(`Skipping JSON schema conversion for invalid schema at path ${path}:`, obj);
        return obj;
      }
      ...
      console.error('Schema object:', JSON.stringify(obj, null, 2));
      console.error('Context:', context);
      // Return the original object instead of crashing
      return obj;
    }
  }
 ...
 *
 * @param generatedCode - The code generated containing placeholders
...
export function restorePlaceholders(
  ...
  for (const placeholder of sortedPlaceholders) {
    ...
    // Since we always use template literals now, always escape for template literals
