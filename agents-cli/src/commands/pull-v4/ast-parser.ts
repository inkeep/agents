...
function analyzePropertyAccessPattern(functionName: string, callExpr: CallExpression): SDKFunction | null {
  // Known environment-aware patterns we should recognize
  const ENVIRONMENT_PATTERNS = {
    'getEnvironmentMcp': { type: 'tools', category: 'environment-aware' },
    'getCredential': { type: 'credentials', category: 'environment-aware' },
    'getEnvironmentCredential': { type: 'credentials', category: 'environment-aware' }
  };
  ...
  if (pattern) {
    return {
      name: functionName,
      category: pattern.category as 'environment-aware',
