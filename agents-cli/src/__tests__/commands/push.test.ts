...
vi.mock('../../utils/state.js', () => ({
  readProjectState: vi.fn().mockReturnValue(undefined),
}));

vi.mock('../../utils/temp-branch.js', () => ({
  withLocalStateBranch: vi.fn(),
}));
...
describe('Push Command - Project Validation', () => {
  ...
  beforeEach(async () => {
    ...
    // Setup initializeCommand mock with default success behavior
    const { initializeCommand } = await import('../../utils/cli-pipeline.js');
...
describe('Push Command - Conflict Detection', () => {
  ...
  function createMockProject(overrides?: Record<string, unknown>) {
    return {
      ...
      getStats: vi.fn().mockReturnValue({ agentCount: 1, tenantId: 'test-tenant' }),
      getAgents: vi.fn().mockReturnValue([]),
      getFullDefinition: vi.fn().mockResolvedValue({ agents: {} }),
      getCredentialTracking: vi.fn().mockResolvedValue({ credentials: {}, usage: {} }),
      ...overrides,
    };
  }
  ...
  it('should detect conflicts and abort push when mergePreview.hasConflicts is true', async () => {
    (existsSync as Mock).mockReturnValue(true);
    const mockProject = createMockProject();
    (importWithTypeScriptSupport as Mock).mockResolvedValueOnce({ default: mockProject });

    const { readProjectState } = await import('../../utils/state.js');
    (readProjectState as Mock).mockReturnValue({
      lastPulledHash: 'abc123',
      lastPulledAt: '2026-01-01',
    });

    const { ManagementApiClient } = await import('../../api.js');
    (ManagementApiClient.create as Mock).mockResolvedValue({});

    const { withLocalStateBranch } = await import('../../utils/temp-branch.js');
    (withLocalStateBranch as Mock).mockResolvedValue({
      hasConflicts: true,
      conflicts: [
        {
          table: 'agents',
    ...
    expect(mockExit).toHaveBeenCalledWith(1);
    expect(mockError).toHaveBeenCalledWith(
      expect.stringContaining('Push aborted: 2 conflict(s) detected')
    );
  });

  it('should proceed with push when no conflicts detected', async () => {
    (existsSync as Mock).mockReturnValue(true);
    const mockProject = createMockProject();
    (importWithTypeScriptSupport as Mock).mockResolvedValueOnce({ default: mockProject });
  ...
  it('should skip conflict check when --force flag is set', async () => {
    (existsSync as Mock).mockReturnValue(true);
    const mockProject = createMockProject();
    (importWithTypeScriptSupport as Mock).mockResolvedValueOnce({ default: mockProject });
    ...
    await pushCommand({ project: '/test/project', force: true });

    expect(withLocalStateBranch).not.toHaveBeenCalled();
    expect(mockProject.init).toHaveBeenCalled();
    expect(mockExit).toHaveBeenCalledWith(0);
  });

  it('should skip conflict check when no lastPulledHash exists', async () => {
    (existsSync as Mock).mockReturnValue(true);
    const mockProject = createMockProject();
    (importWithTypeScriptSupport as Mock).mockResolvedValueOnce({ default: mockProject });

    const { readProjectState } = await import('../../utils/state.js');
    (readProjectState as Mock).mockReturnValue(undefined);
  ...
  it('should proceed with push when conflict check throws error', async () => {
    (existsSync as Mock).mockReturnValue(true);
    const mockProject = createMockProject();
    (importWithTypeScriptSupport as Mock).mockResolvedValueOnce({ default: mockProject });
    ...
    const { withLocalStateBranch } = await import('../../utils/temp-branch.js');
    (withLocalStateBranch as Mock).mockRejectedValue(new Error('API unavailable'));
