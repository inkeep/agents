...
- **Setup (core)**: `pnpm setup-dev` — core DBs only (Doltgres, Postgres, SpiceDB)
- **Setup (full)**: `pnpm setup-dev:full` — core DBs + Nango + SigNoz + OTEL + Jaeger (auto-wires all env vars)
- **Optional services**: `pnpm setup-dev:stop` | `pnpm setup-dev:status` | `pnpm setup-dev:reset`
