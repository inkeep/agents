...
- **Tests**: `pnpm --filter @inkeep/agents-api test` runs the full Vitest suite including PGlite setup. Individual tests can be run with `npx vitest --run src/__tests__/<file>.test.ts` from the `agents-api` directory. The `--run` flag is already in the `test` script — do NOT pass `--run` via `pnpm test --run` (causes "Expected a single value" error). Use `pnpm test` or `npx vitest --run <file>` directly.
