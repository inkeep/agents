...
const {
  ...
  ManagementApiClientMock: vi.fn(() => ({
    listEvaluationRunConfigs: vi.fn(),
    getEvaluationSuiteConfigById: vi.fn(),
    getEvaluationSuiteConfigEvaluatorRelations: vi.fn(),
...
describe('Evaluation Triggers - Unit Tests', () => {
  ...
  describe('ManagementApiClient mock behavior', () => {
    it('should return active evaluation run configs', async () => {
      ...
      mockClient.listEvaluationRunConfigs.mockResolvedValue(mockConfigs);

      const result = await mockClient.listEvaluationRunConfigs();
    ...
    it('should return suite config with evaluator relations', async () => {
      ...
      const suiteConfig = await mockClient.getEvaluationSuiteConfigById('suite-1');
      const relations = await mockClient.getEvaluationSuiteConfigEvaluatorRelations('suite-1');
