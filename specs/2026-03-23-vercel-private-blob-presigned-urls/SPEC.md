# Presigned URLs with Vercel Private Blob Storage — Spec
...
- G2: ~~Determine the optimal delivery architecture~~ ✅ Decided — Option C (S3 presigned URLs)
...
- G6: Add `getPresignedUrl()` to `BlobStorageProvider` interface with S3 + MinIO support
- G7: Provide local development story via MinIO in Docker
...
## 9) Proposed solution — Architecture options
...
### Option C: S3-Compatible Presigned URLs (bypass Vercel Blob for reads)
...
### Option D: Hybrid — HMAC proxy for Vercel Blob, native presigned for S3

Extend the existing spec with backend-aware URL generation:
- When backend is S3: generate native S3 presigned URLs (direct client-to-S3)
- When backend is Vercel Blob: generate HMAC-signed proxy URLs (same as Option A)
- `resolveMessageBlobUris()` checks which backend is active and generates the appropriate URL

**Pros:**
- Best performance when S3 is available
- Falls back gracefully to proxy pattern for Vercel Blob
- Future-proof: if Vercel adds presigned URLs, easy to add a third path

**Cons:**
- More complex URL generation logic
- Different URL formats depending on backend (clients shouldn't care, but it's observable)
- S3 presigned URLs leak bucket info
...
| Must | R4 | Manage proxy backward compat | Existing `/manage/.../media/{key}` route remains functional | Fallback for Vercel Blob and local backends |
| Must | R5 | Local dev with presigned URLs | Adobe S3Mock in `docker-compose.dbs.yml` with S3-compatible presigned URL support | Presigned URLs work end-to-end in local dev |
| Must | R6 | 1-hour default expiry | Presigned URLs expire after 3600 seconds | Configurable via env var |
| Should | R7 | Unit tests for presigned URL generation | Tests for S3 presigned path, fallback path, expiry behavior | |
| Should | R8 | Update existing resolve-blob-uris tests | Adapt to async signatures, test both S3 and proxy URL outcomes | |

## 5) Proposed solution — Implementation design

**Decision: Option C — S3-compatible presigned URLs.** S3 is the storage backend for private media in production. MinIO provides the same presigned URL capability for local development.

### 5.1 Interface change: `BlobStorageProvider.getPresignedUrl()`
...
### 5.2 S3 presigned URL generation
...
### 5.3 Update `resolveMessageBlobUris()` → async
...
### 5.5 Local development: Adobe S3Mock

[Adobe S3Mock](https://github.com/adobe/S3Mock) is an S3-compatible mock server that supports presigned URLs. The existing `S3BlobStorageProvider` already handles custom endpoints via `BLOB_STORAGE_S3_ENDPOINT` and `forcePathStyle`.

**Why S3Mock over alternatives:**
- Apache 2.0, actively maintained by Adobe (~96MB Docker image)
- Accepts presigned URLs generated by `@aws-sdk/s3-request-presigner` — routes correctly to objects
- Zero config: runs with a single `docker run` command
- ~~MinIO~~ deprecated; ~~LocalStack~~ repo archived, requires auth token; ~~s3rver~~ abandoned

**Caveat:** S3Mock accepts but does **not enforce** presigned URL expiry or signature validation. This is fine for local dev — the code path exercising `getSignedUrl()` is identical. Signature enforcement is tested in production against real S3.

**Add to `docker-compose.dbs.yml`:**

```yaml
s3mock:
  image: adobe/s3mock:latest
  ports:
    - "9090:9090"   # S3 API (HTTP)
  environment:
    initialBuckets: inkeep-media
  volumes:
    - s3mock-data:/s3mockroot
```

S3Mock auto-creates buckets listed in `initialBuckets` on startup — no init container or script needed.

**Default `.env` for local dev:**

```env
BLOB_STORAGE_S3_BUCKET=inkeep-media
BLOB_STORAGE_S3_REGION=us-east-1
BLOB_STORAGE_S3_ACCESS_KEY_ID=S3MOCK
BLOB_STORAGE_S3_SECRET_ACCESS_KEY=S3MOCK
BLOB_STORAGE_S3_ENDPOINT=http://localhost:9090
BLOB_STORAGE_S3_FORCE_PATH_STYLE=true
```

### 5.6 Manage proxy — backward compatibility

The existing `/manage/tenants/{tenantId}/projects/{projectId}/conversations/{id}/media/{mediaKey}` route is **kept as-is**:

- Continues to work for Vercel Blob and local backends (which don't support presigned URLs)
...
| `docker-compose.dbs.yml` | Add MinIO service | Modify |
| `.env.example` | Document MinIO defaults for local dev | Modify |
...
| D6 | Option C — S3 presigned URLs (no proxy for reads) | X | DECIDED | No | Direct client-to-S3, zero function invocations for media reads. |
...
| D8 | Keep `/manage` media proxy for backward compatibility | X | DECIDED | No | Existing route remains as fallback for Vercel Blob / local backends. |
| D9 | Adobe S3Mock for local dev presigned URL support | X | DECIDED | No | S3-compatible, Apache 2.0, ~96MB image, auto-creates buckets, existing provider supports custom endpoints + forcePathStyle. MinIO deprecated, LocalStack archived. |
| D10 | `getPresignedUrl()` is optional on `BlobStorageProvider` interface | T | DECIDED | No | Only S3/MinIO implement it; Vercel Blob and local providers fall back to manage proxy. |

## 11) Open questions

| ID | Question | Type | Priority | Blocking? | Status |
|---|---|---|---|---|---|
| Q1 | ~~Which architecture option (A/B/C/D)?~~ | X | P0 | ~~Yes~~ | ✅ Resolved → D6: Option C |
...
| Q4 | ~~Bucket creation strategy~~ | T | ~~P1~~ | ~~No~~ | ✅ Resolved — S3Mock `initialBuckets` env var auto-creates buckets on startup, no script needed |
