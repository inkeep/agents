...
All documentation in `/agents-docs/content/docs/` must follow our style guide:

**Quick Reference:**
- **Titles:** Use `<verb> <noun> <context>` pattern (e.g., "Connect to Native MCP Servers")
- **sidebarTitle:** Keep to 1-3 words for sidebar navigation
- **Frontmatter:** Include title, sidebarTitle, description, icon, keywords
- **File paths:** Must match URL structure exactly (kebab-case)
- **Navigation:** Update `/agents-docs/navigation.ts` when adding pages
- **Redirects:** Add to `/agents-docs/redirects.json` when moving/renaming

**Terminology Standards:**
- MCP servers: "Native", "Composio's out-of-box MCP servers", "Custom"
- CLI templates: "Add templates" (not "scaffold")
- Be explicit: "no building required" vs "build and deploy"
- Use active voice and action verbs

**Detailed Guidelines:** See `.cursor/rules/documentation-style-guide.mdc` for comprehensive style guide including:
- Title patterns and sidebar requirements
- Terminology standards (MCP servers, CLI functionality)
- User experience patterns (decision guidance, avoid conflation)
- File organization and frontmatter requirements
- Recent terminology decisions documentation
