...
export class AgentSession {
  ...
  private readonly MAX_ARTIFACT_RETRIES = runtimeConfig.ARTIFACT_GENERATION_MAX_RETRIES;
  private readonly MAX_PENDING_ARTIFACTS = runtimeConfig.ARTIFACT_SESSION_MAX_PENDING; // Prevent unbounded growth
  ...
  initializeStatusUpdates(
    ...
    this.statusUpdateState = {
      ...
      baseModel,
      config: {
        numEvents: config.numEvents || runtimeConfig.STATUS_UPDATE_DEFAULT_NUM_EVENTS,
        timeInSeconds: config.timeInSeconds || runtimeConfig.STATUS_UPDATE_DEFAULT_INTERVAL_SECONDS,
        ...config,
      },
    };
  ...
  private async generateAndSendUpdate(): Promise<void> {
    ...
    try {
      ...
      if (this.previousSummaries.length > runtimeConfig.ARTIFACT_SESSION_MAX_PREVIOUS_SUMMARIES) {
                  ...
                  try {
                    ...
                    if (attempt < maxRetries) {
                      const backoffMs = Math.min(
                        runtimeConfig.ARTIFACT_GENERATION_BACKOFF_INITIAL_MS * 2 ** (attempt - 1),
                        runtimeConfig.ARTIFACT_GENERATION_BACKOFF_MAX_MS
                      );
