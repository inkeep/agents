...
export const getCoordinates = functionTool({
	...
	description: 'Convert a location name or address into geographic coordinates (latitude and longitude)',
	inputSchema: {
		type: 'object',
		properties: {
			location: {
	...
	execute: async ({ location }: { location: string }) => {
		...
		return { latitude: 40.7128, longitude: -74.006, name: `${location} (defaulted to New York)`, found: false };
	},
});
