...
/**
 * Activities Planner Advanced Agent
 *
 ...
 * 4. Using the calculate-activity-score tool to calculate a score for one of the activities based on the weather conditions
 *
 * Example usage:
 ...
 * 
 * Additional features: 
 * - Fetches time information and timezone from request headers, enabling users to ask about the current time and receive time-aware activity recommendations. 
 * - Requires passing the timezone in headers as "tz": "US/Pacific".
 *   - Example: "What is the current time in US/Pacific?"
 *   - Example: "What are some good activities in two hours?"
 * - Uses status updates to track the progress of the agent and the activities planner agent.
 * - Uses artifact components to save sources generated by the websearch MCP tool.
*/
...
const timeFetcher = fetchDefinition({
  ...
  trigger: "invocation",
  fetchConfig: {
    url: `https://world-time-api3.p.rapidapi.com/timezone/${headersSchema.toTemplate(
      "tz"
    )}`,
    method: "GET",
    /**
     * Note: API keys should typically be stored in environment variables (.env) rather than hardcoded.
     * For this example, a free API key is used directly for simplicity.
     */
    headers: {
...
const activitiesPlannerContext = contextConfig({
...
const activitiesPlanner = subAgent({
  ...
  prompt: `You are a proactive, helpful assistant. The time is ${activitiesPlannerContext.toTemplate(
    "time"
