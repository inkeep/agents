...
import type { ConflictItem, EvalSummaryResult } from '@inkeep/agents-core';
import {
  ...
  ImprovementDiffQuerySchema,
  ImprovementDiffResponseSchema,
  ImprovementListResponseSchema,
  ImprovementMergeRequestSchema,
  ImprovementMergeResponseSchema,
  ImprovementRevertRequestSchema,
  ImprovementSuccessMessageSchema,
  ImprovementTriggerRequestSchema,
...
app.openapi(
  ...
  async (c) => {
    ...
    const projectMainRef = `${tenantId}_${projectId}_main`;

    const seen = new Set<string>();
    const data: ImprovementListEntry[] = [];
    for (const run of runs) {
      const name = run.ref?.name;
      if (!name || name === projectMainRef || seen.has(name)) continue;
      seen.add(name);
      data.push({
        branchName: name,
        status: run.status,
  ...
  createProtectedRoute({
    ...
    request: {
      ...
      body: {
        content: {
          'application/json': {
            schema: ImprovementTriggerRequestSchema,
          },
        },
      },
    },
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: ImprovementTriggerResponseSchema,
          },
        },
      },
      ...commonGetErrorResponses,
...
app.openapi(
  ...
  async (c) => {
    ...
    return c.json({ id: run.id, conversationId: run.conversationId });
  }
);
...
app.openapi(
  createProtectedRoute({
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: ImprovementDiffResponseSchema,
          },
        },
      },
      ...commonGetErrorResponses,
  ...
  async (c) => {
    ...
    if (decodedBranchName === 'main') {
      throw createApiError({
        code: 'bad_request',
        message: 'Cannot operate on main branch',
      });
    }
  ...
  createProtectedRoute({
    ...
    request: {
      ...
      body: {
        content: {
          'application/json': {
            schema: ImprovementMergeRequestSchema,
          },
        },
        required: false,
      },
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: ImprovementMergeResponseSchema,
          },
        },
      },
      409: {
  ...
  async (c) => {
    ...
    if (mergeResponse.status === 409) {
      const conflicts = await buildImprovementConflictItems(
        ...
        logger.warn(
          { err, sourceBranch: decodedBranchName, targetBranch },
          'Failed to build conflict items for 409 merge response'
        );
        return [];
      });
    ...
    if (!mergeResponse.ok) {
      ...
      const message =
        errorBody?.error?.message ?? `Merge failed with status ${mergeResponse.status}`;
    ...
    await deleteCoPilotRunsByBranchName(runDbClient)({
      scopes: { tenantId, projectId },
      branchName: decodedBranchName,
    }).catch((err: unknown) => {
      logger.warn(
        { err, branchName: decodedBranchName },
        'Failed to delete copilot runs after merge'
      );
    });
...
app.openapi(
  createProtectedRoute({
    ...
    request: {
      ...
      body: {
        content: {
          'application/json': {
            schema: ImprovementRevertRequestSchema,
          },
        },
      },
    },
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: ImprovementSuccessMessageSchema,
          },
        },
      },
      ...commonGetErrorResponses,
  ...
  async (c) => {
    ...
    const result = await revertImprovementRows(db)({
      sourceBranchFullName: branchFullName,
      targetBranchFullName: targetFullName,
      rows,
      commitMessage: `Revert ${rows.length} excluded row(s) before merge into ${targetBranch}`,
    });
...
app.openapi(
  ...
  async (c) => {
    ...
    await deleteCoPilotRunsByBranchName(runDbClient)({
      ...
      logger.warn(
        { err, branchName: decodedBranchName },
        'Failed to delete copilot runs after reject'
      );
    });
...
app.openapi(
  createProtectedRoute({
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: ImprovementConversationResponseSchema,
          },
        },
      },
      ...commonGetErrorResponses,
  ...
  async (c) => {
    ...
    const runs = await listCoPilotRunsByBranchName(runDbClient)({
      scopes: { tenantId, projectId },
      branchName: decodedBranchName,
    });

    if (runs.length === 0) {
      return c.json({ conversationIds: [], status: undefined, feedbackItems: undefined });
    }

    const oldest = runs[0];
    const newest = runs[runs.length - 1];
    const feedbackIds = oldest.feedbackIds ?? [];
    ...
    const feedbackItems =
      feedbackIds.length > 0
        ? await getFeedbackByIds(runDbClient)({
            ...
            logger.warn(
              { err, feedbackIds, tenantId, projectId, branchName: decodedBranchName },
              'Failed to fetch feedback items for improvement'
            );
            return [];
          })
        : [];
    ...
    return c.json({
      conversationIds: runs.map((r) => r.conversationId),
...
app.openapi(
  ...
  async (c) => {
    ...
    const branchRuns = allRuns.filter((run) => {
      ...
      return refName === improvementBranchFullName;
    });

    await doltCheckout(db)({ branch: improvementBranchFullName });

    type EvaluationResultRow = Awaited<
      ReturnType<ReturnType<typeof listEvaluationResultsByRun>>
    >[number];
    ...
    const runData = await Promise.all(
      branchRuns.map(async (run) => {
        ...
        let allResults: EvaluationResultRow[] = [];
    ...
    for (const entry of runData) {
      for (const result of entry.allResults) {
        allEvaluatorIds.add(result.evaluatorId);
      }
    }

    const evaluators =
      allEvaluatorIds.size > 0
        ? await getEvaluatorsByIds(db)({
            scopes: { tenantId, projectId },
            evaluatorIds: Array.from(allEvaluatorIds),
          })
        : [];
    const evaluatorMap = new Map(evaluators.map((e) => [e.id, e]));

    const datasetRuns = runData.map(({ run, dataset, statusCounts, allResults, inputMap }) => {
      const evaluationResults: EvalSummaryResult[] = allResults.map((result) => {
