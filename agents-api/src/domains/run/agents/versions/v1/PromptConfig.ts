...
{ "$tool": "call_id" } passes the entire raw output. If the next tool needs only a specific field,
use an intermediate extraction step — never read the value and copy it inline.
  ...
  Step 2: extract({ "source": { "$tool": "call_a" }, ... })  → returns "..."  (call_id: "call_b")
  Step 3: tool_b({ "input": { "$tool": "call_b" } })  ← receives the extracted value
...
✅ CORRECT — extract first, then reference:
  ...
  extract({ "source": { "$tool": "call_a" }, ... })  (call_id: "call_b")
...
When to use each:
✅ Use { "$tool": "call_id" } when the next tool can accept the full output of the previous tool
...
AFTER AN EXTRACTION STEP — reference the extraction's call_id, not the original source:
  ...
  extract({ "data": { "$tool": "call_source" }, ... })  (call_id: "call_extract")
  next_tool({ "input": { "$tool": "call_extract" } })  ← use call_extract, NOT call_source

❌ WRONG — referencing the original source after extraction:
  next_tool({ "input": { "$tool": "call_source" } })  ← skips the extraction, passes the full object again
...
extraction step on it. An extraction applied to a plain string or number returns null.
  ...
  next_tool({ "input": { "$tool": "call_extract" } })  ← correct, receives the string directly
  ❌ extract({ "data": { "$tool": "call_extract" }, ... })  ← wrong, cannot extract from a primitive`;
  }
