...
export async function callLlmStep(params: CallLlmStepParams): Promise<CallLlmResult> {
  ...
  if (outputFormat === 'vercel') {
    ...
    logger.debug({ requestId }, 'callLlmStep: using Vercel stream writer');
  } else {
    ...
    logger.debug({ requestId }, 'callLlmStep: using SSE stream writer');
  }
  ...
  logger.debug(
    { requestId, conversationId, currentSubAgentId },
    'callLlmStep: session and stream registered'
  );
  ...
  try {
    ...
    // After a tool approval round, send a continuation prompt instead of the original
    // user message. This relies on the delegation tool result being persisted to
    // conversation history (via the `skipHistoryStorage` fix in tool-wrapper.ts) so the
    // LLM sees the tool output in its context window.
...
export async function executeToolStep(params: ExecuteToolStepParams): Promise<ExecuteToolResult> {
  ...
  logger.info(
    { toolName, toolCallId, currentSubAgentId, workflowRunId, streamNamespace },
    'executeToolStep starting'
  );
  ...
  if (outputFormat === 'vercel') {
    ...
    closeable = vercelWriter;
    sseHelper = createVercelStreamHelper(vercelWriter);
    logger.debug({ requestId, toolName }, 'executeToolStep: using Vercel stream writer');
  } else {
    const writableStream = new WritableBackedHonoSSEStream(writable);
    closeable = writableStream;
    sseHelper = createSSEStreamHelper(writableStream, requestId, timestamp);
    logger.debug({ requestId, toolName }, 'executeToolStep: using SSE stream writer');
  }
