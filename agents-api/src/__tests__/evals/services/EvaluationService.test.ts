...
const {
  ...
  resolveRefMock,
  withRefMock,
  getEvaluationJobConfigByIdInnerMock,
  getEvaluationJobConfigEvaluatorRelationsInnerMock,
  getEvaluatorByIdInnerMock,
  ModelFactoryMock,
} = vi.hoisted(() => ({
  ...
  getEvaluationJobConfigByIdInnerMock: vi.fn(),
  getEvaluationJobConfigEvaluatorRelationsInnerMock: vi.fn(),
  getEvaluatorByIdInnerMock: vi.fn(),
  ModelFactoryMock: {
...
vi.mock('@inkeep/agents-core', async (importOriginal) => {
  ...
  return {
    ...
    withRef: withRefMock,
    getEvaluationJobConfigById: vi.fn(() => getEvaluationJobConfigByIdInnerMock),
    getEvaluationJobConfigEvaluatorRelations: vi.fn(
      () => getEvaluationJobConfigEvaluatorRelationsInnerMock
    ),
    getEvaluatorById: vi.fn(() => getEvaluatorByIdInnerMock),
    ModelFactory: ModelFactoryMock,
    InternalServices: {
      ...(actual as any).InternalServices,
...
describe('EvaluationService', () => {
  ...
  describe('runDatasetItem', () => {
    ...
    it('should run single-turn conversation successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        'http://localhost:3003/run/api/chat',
  ...
  describe('runEvaluationJob', () => {
    beforeEach(() => {
      getEvaluationJobConfigByIdInnerMock.mockReset();
      getEvaluationJobConfigEvaluatorRelationsInnerMock.mockReset();
      getEvaluatorByIdInnerMock.mockReset();
    });

    it('should throw error when evaluation job config not found', async () => {
      getEvaluationJobConfigByIdInnerMock.mockResolvedValue(null);
    ...
    it('should throw error when no evaluators found', async () => {
      getEvaluationJobConfigByIdInnerMock.mockResolvedValue({
        id: 'config-1',
        jobFilters: {},
      });
      getEvaluationJobConfigEvaluatorRelationsInnerMock.mockResolvedValue([]);
    ...
    it('should throw error when all evaluators are invalid', async () => {
      ...
      getEvaluatorByIdInnerMock.mockResolvedValue(null);
    ...
    it('should return empty results when no conversations match filters', async () => {
      ...
      getEvaluatorByIdInnerMock.mockResolvedValue({
        id: 'eval-1',
        name: 'Test Evaluator',
      });
