...
describe('INKEEP_MCP_ALLOWED_TOOLS', () => {
  it('is the curated 167-tool golden path with no duplicates', () => {
    expect(INKEEP_MCP_ALLOWED_TOOLS.length).toBe(167);
    expect(new Set(INKEEP_MCP_ALLOWED_TOOLS).size).toBe(167);
  });
  ...
  it('passing the allowlist to createMCPServer exposes exactly those 167 tools', () => {
    const exposed = registeredToolNames({ allowedTools: [...INKEEP_MCP_ALLOWED_TOOLS] });
    expect(exposed.size).toBe(167);
