...
describe('InlineDocumentDataSchema - office document MIME normalization', () => {
  ...
  it('accepts lowercase pptx data URI', () => {
    const input = `data:${PPTX_MIME};base64,${VALID_ZIP_BASE64}`;
    expect(InlineDocumentDataSchema.safeParse(input).success).toBe(true);
  });

  it('accepts lowercase pages data URI', () => {
    const input = `data:${PAGES_MIME};base64,${VALID_ZIP_BASE64}`;
    expect(InlineDocumentDataSchema.safeParse(input).success).toBe(true);
  });
  ...
  it('includes supported ZIP extensions in office data URI validation messages', () => {
    const result = InlineDocumentDataSchema.safeParse('data:application/zip;base64,UEsDBA==');
    expect(result.success).toBe(false);
    if (result.success) {
      throw new Error('Expected office document validation to fail');
    }

    expect(result.error.issues[0]?.message).toContain('.docx');
...
describe('VercelFilePartSchema - office document URL normalization', () => {
  ...
  it('accepts lowercase pptx data URI in url field', () => {
    const part = {
      type: 'file',
      url: `data:${PPTX_MIME};base64,${VALID_ZIP_BASE64}`,
      mediaType: PPTX_MIME,
    };
    expect(VercelFilePartSchema.safeParse(part).success).toBe(true);
  });

  it('accepts lowercase pages data URI in url field', () => {
    const part = {
      type: 'file',
      url: `data:${PAGES_MIME};base64,${VALID_ZIP_BASE64}`,
      mediaType: PAGES_MIME,
    };
    expect(VercelFilePartSchema.safeParse(part).success).toBe(true);
  });
  ...
  it('rejects office document with external URL', () => {
    ...
    const result = VercelFilePartSchema.safeParse(part);
    expect(result.success).toBe(false);
    if (result.success) {
      throw new Error('Expected office document URL validation to fail');
    }
