...
describe('uploadPartsImages', () => {
  ...
  it('calls downloadExternalImage for URI file parts', async () => {
    ...
    expect(mockUpload).toHaveBeenCalledWith(
      expect.objectContaining({
        key: expect.stringContaining('tenant/project/conversation/message/0-'),
  ...
  it('calls normalizeInlineImageBytes for inline byte file parts', async () => {
    const parts = [
  ...
  it('preserves non-file parts and metadata while uploading files', async () => {
    ...
    expect(uploaded[1]).toMatchObject({
      kind: 'file',
      file: {
        uri: expect.stringContaining('blob://tenant/project/conversation/message/1-'),
        mimeType: 'image/png',
      },
      metadata: { source: 'user-upload' },
    });
