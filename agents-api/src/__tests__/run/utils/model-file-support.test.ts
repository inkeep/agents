...
describe('supportsOfficeDocuments', () => {
  ...
  it('returns true for azure/ prefix', () => {
    expect(supportsOfficeDocuments('azure/gpt-4o')).toBe(true);
  });
...
describe('stripIncompatibleOfficeParts', () => {
  it('returns all parts compatible for OpenAI model', () => {
    const parts = [
      ...
      makeFilePart(PPTX_MIME, 'deck.pptx'),
    ];
  ...
  it('strips pptx part for Anthropic model', () => {
    const parts = [makeFilePart(PPTX_MIME, 'deck.pptx')];
    const { compatible, stripped } = stripIncompatibleOfficeParts(
      parts,
      'anthropic/claude-3-5-sonnet-20241022'
    );
    expect(compatible).toHaveLength(0);
    expect(stripped).toHaveLength(1);
    expect(stripped[0]).toEqual({ mimeType: PPTX_MIME, filename: 'deck.pptx' });
  });

  it('strips pages part for Anthropic model', () => {
    const parts = [makeFilePart(PAGES_MIME, 'proposal.pages')];
    const { compatible, stripped } = stripIncompatibleOfficeParts(
      parts,
      'anthropic/claude-3-5-sonnet-20241022'
    );
    expect(compatible).toHaveLength(0);
    expect(stripped).toHaveLength(1);
    expect(stripped[0]).toEqual({ mimeType: PAGES_MIME, filename: 'proposal.pages' });
  });
...
describe('buildStrippedPartsNote', () => {
  it('includes filename when present', () => {
    const note = buildStrippedPartsNote([{ mimeType: DOCX_MIME, filename: 'report.docx' }]);
    expect(note).toContain('"report.docx"');
    expect(note).toContain(DOCX_MIME);
    expect(note).toContain('not supported in the current configuration');
    expect(note).toContain('Please tell the user you are unable to access this file');
  });

  it('uses (unnamed) when filename is absent', () => {
    const note = buildStrippedPartsNote([{ mimeType: DOCX_MIME, filename: undefined }]);
