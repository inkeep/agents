...
describe('OpenAPI Specification - Integration Tests', () => {
  describe('GET /openapi.json', () => {
    ...
    it('should have correct API metadata', async () => {
      ...
      expect(spec.info.title).toBe('Inkeep Agents API');
    ...
    it('should contain path definitions', async () => {
      ...
      // Check for key run API endpoints (mounted under /run)
      expect(spec.paths).toHaveProperty('/run/v1/chat/completions');
    });
    ...
    it('should have response definitions for operations', async () => {
      ...
      const chatPath = spec.paths['/run/v1/chat/completions'];
    ...
    it('should include chat completions endpoint', async () => {
      ...
      const chatEndpoint = spec.paths['/run/v1/chat/completions'];
