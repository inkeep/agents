...
describe('Ref Middleware - Integration Tests', () => {
  ...
  const createDoltTag = async (tagName: string, branchName: string) => {
    ...
    await doltTag(manageDbClient)({ name: tagName, revision: branchName });
  };
  ...
  describe('refMiddleware - Project Main Branch', () => {
    it('should auto-create project main branch when creating a project', async () => {
      ...
      const branches = await doltBranchExists(manageDbClient)({ name: projectMain });

      expect(branches).toBe(true);
    });
  ...
  describe('refMiddleware - Custom Ref Resolution', () => {
    it('should resolve custom branch ref when provided', async () => {
      ...
      const res = await makeRequest(`/manage/tenants/${tenantId}/projects/${projectId}?ref=custom`);
    ...
    it('should resolve tag ref when provided', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}?ref=${tagName}`
      );
      expect(res.status).toBe(200);
    });
    ...
    it('should return 404 for unknown ref', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}?ref=non-existent-ref`
      );
  ...
  describe('writeProtectionMiddleware - Branch Writes', () => {
    it('should allow POST requests to branches', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}?ref=writeable`,
        {
          method: 'POST',
    ...
    it('should allow DELETE requests to branches', async () => {
      ...
      const res1 = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/branches?ref=writeable`,
        {
          method: 'POST',
      ...
      const res2 = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/branches/temp-branch?ref=${customBranch}`,
        {
          method: 'DELETE',
        }
      );
