  ...
  describe('GET /', () => {
    it('should list artifact components with pagination (empty initially)', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/artifact-components?page=1&limit=10`
      );
    ...
    it('should handle pagination with multiple pages (small page size)', async () => {
      ...
      const firstPageRes = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/artifact-components?page=1&limit=${PAGE_SIZE}`
      );
      ...
      const lastPageRes = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/artifact-components?page=3&limit=${PAGE_SIZE}`
      );
    ...
    it('should validate maximum page size to 100', async () => {
      ...
      await createTestProject(manageDbClient, tenantId, projectId);
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/artifact-components?limit=1000`
      );
  ...
  describe('DELETE /{id}', () => {
    ...
    it('should return 404 for artifact component in different tenant', async () => {
      ...
      const verifyRes = await makeRequest(
        `/manage/tenants/${tenantId1}/projects/${projectId}/artifact-components/${artifactComponentId}`
      );
