  ...
  describe('GET /', () => {
    it('should list API keys with pagination (empty initially)', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/api-keys?page=1&limit=10`
      );
    ...
    it('should filter API keys by agentId', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/api-keys?agentId=${agent1}`
      );
      expect(res.status).toBe(200);
    ...
    it('should handle pagination correctly', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/api-keys?page=1&limit=3`
      );
      expect(res.status).toBe(200);
