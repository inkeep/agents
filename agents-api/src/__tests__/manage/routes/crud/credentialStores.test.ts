    ...
    default: {
      request: vi.fn(async (url: string, options: any) => {
        ...
        const app = createAgentsHono({
          ...
          credentialStores: currentRegistry,
          auth: null,
        });
...
describe('Credential Stores - CRUD Operations', () => {
  ...
  describe('GET /stores', () => {
    it('should return all available stores when all are functional', async () => {
      ...
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores`,
        { method: 'GET' }
      );
  ...
  describe('POST /stores/:storeId/credentials', () => {
    ...
    it('should successfully create a credential in the store', async () => {
      ...
      // Act
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores/${storeId}/credentials`,
        {
          method: 'POST',
    ...
    it('should return 500 when credential store is not available', async () => {
      ...
      // Act
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores/unavailable-store/credentials`,
        {
          method: 'POST',
          body: JSON.stringify(requestBody),
        }
      ...
      expect(data.error.message).toContain(
        "[REDACTED] store 'unavailable-store' is not available: Store is offline"
      );
    });
    ...
    it('should handle store.set() errors gracefully', async () => {
      ...
      // Act
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores/error-store/credentials`,
        {
          method: 'POST',
          body: JSON.stringify(requestBody),
        }
      ...
      expect(response.status).toBe(500);
      const data = await response.json();
      expect(data.error.message).toBe('Failed to store [REDACTED]: Storage failed');
    });
    ...
    it('should handle keychain store type', async () => {
      ...
      // Act
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores/keychain-store/credentials`,
        {
          method: 'POST',
          body: JSON.stringify(requestBody),
        }
    ...
    it('should handle nango store type', async () => {
      ...
      // Act
      const response = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/credential-stores/nango-store/credentials`,
        {
          method: 'POST',
          body: JSON.stringify(requestBody),
        }
