...
describe('Evaluation Results CRUD Routes - Integration Tests', () => {
  ...
  describe('GET /{resultId}', () => {
    ...
    it('should return 404 when result not found', async () => {
      ...
      await createTestProject(manageDbClient, tenantId, projectId);
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-results/non-existent-id`
      );
  ...
  describe('POST /', () => {
    it('should create a new evaluation result', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-results`,
        {
          method: 'POST',
