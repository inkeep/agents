...
describe('Evaluation Run Configs CRUD Routes - Integration Tests', () => {
  ...
  const createTestRunConfig = async ({
    ...
    const createRes = await makeRequest(
      `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-run-configs`,
      {
        method: 'POST',
  ...
  describe('GET /{configId}', () => {
    it('should get a run config by id', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-run-configs/${configId}`
      );
      expect(res.status).toBe(200);
      const body = await res.json();
    ...
    it('should return 404 when run config not found', async () => {
      ...
      await createTestProject(manageDbClient, tenantId, projectId);
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-run-configs/non-existent-id`
      );
  ...
  describe('GET /{configId}/results', () => {
    it('should return empty results when no evaluation runs exist', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/evals/evaluation-run-configs/${configId}/results`
      );
      expect(res.status).toBe(200);
      const body = await res.json();
      expect(body.data).toEqual([]);
