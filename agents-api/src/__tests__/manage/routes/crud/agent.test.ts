...
import {
  createTestAgentToolRelationData,
...
describe('Agent CRUD Routes - Integration Tests', () => {
  ...
  describe('GET /', () => {
    it('should list agents with pagination (empty initially)', async () => {
      ...
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/agents?page=1&limit=10`
      );
    ...
    it('should handle pagination with multiple pages (small page size)', async () => {
      ...
      const page1Res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/agents?page=1&limit=2`
      );
      ...
      const page2Res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/agents?page=2&limit=2`
      );
  ...
  describe('GET /{agentId}/full', () => {
    ...
    it('should return 404 when agent not found', async () => {
      ...
      await createTestProject(manageDbClient, tenantId, projectId);
      const res = await makeRequest(
        `/manage/tenants/${tenantId}/projects/${projectId}/agents/non-existent-agent/full`
      );
      expect(res.status).toBe(404);
    ...
    it('should include multiple agents when agent has relationships', async () => {
      ...
      try {
        await makeRequest(`/manage/tenants/${tenantId}/projects/${projectId}/agent-relations`, {
          method: 'POST',
          body: JSON.stringify({
            agentId,
