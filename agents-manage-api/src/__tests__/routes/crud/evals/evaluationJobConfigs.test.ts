...
describe('Evaluation Job Configs CRUD Routes - Integration Tests', () => {
  ...
  describe('GET /{configId}', () => {
    ...
    it('should return 404 when job config not found', async () => {
      ...
      await createTestProject(manageDbClient, tenantId, projectId);
      const res = await makeRequest(
        `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/non-existent-id`
      );
  ...
  describe('POST /', () => {
    ...
    it('should create job config with multiple evaluators', async () => {
      ...
      const relationsRes = await makeRequest(
        `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/${body.data.id}/evaluators`
      );
  ...
  describe('Evaluator Relations', () => {
    ...
    describe('POST /{configId}/evaluators/{evaluatorId}', () => {
      it('should add evaluator to job config', async () => {
        ...
        const res = await makeRequest(
          `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/${configId}/evaluators/${evalId2}`,
          { method: 'POST' }
        );
        expect(res.status).toBe(201);
    ...
    describe('DELETE /{configId}/evaluators/{evaluatorId}', () => {
      it('should remove evaluator from job config', async () => {
        ...
        const res = await makeRequest(
          `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/${configId}/evaluators/${evaluatorId}`,
          { method: 'DELETE' }
        );
        expect(res.status).toBe(204);
      ...
      it('should return 404 when relation not found', async () => {
        ...
        const res = await makeRequest(
          `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/${configId}/evaluators/non-existent-evaluator`,
          { method: 'DELETE' }
        );
        expect(res.status).toBe(404);
      });
  ...
  describe('End-to-End Workflow', () => {
    it('should complete full job config lifecycle', async () => {
      ...
      const removeRes = await makeRequest(
        `/tenants/${tenantId}/projects/${projectId}/evals/evaluation-job-configs/${configId}/evaluators/${evalId1}`,
        { method: 'DELETE' }
      );
