...
app.openapi(
  createRoute({
    ...
    tags: ['Evaluations'],
    request: {
      params: TenantProjectIdParamsSchema.extend({ configId: z.string() }),
    },
    responses: {
      200: {
...
app.openapi(
  ...
  async (c) => {
    ...
    try {
      const id = (configData as any).id || generateId();
...
app.openapi(
  ...
  async (c) => {
    ...
    try {
      ...
      const jobRuns = evaluationRuns.filter(
        (run) => run.evaluationJobConfigId === configId
      );
