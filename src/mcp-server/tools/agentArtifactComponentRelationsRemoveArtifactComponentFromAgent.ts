/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: 8ba285d7207a
 */

import { agentArtifactComponentRelationsRemoveArtifactComponentFromAgent } from "../../funcs/agentArtifactComponentRelationsRemoveArtifactComponentFromAgent.js";
import { RemoveArtifactComponentFromAgentRequest$zodSchema } from "../../models/removeartifactcomponentfromagentop.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: RemoveArtifactComponentFromAgentRequest$zodSchema,
};

export const tool$agentArtifactComponentRelationsRemoveArtifactComponentFromAgent:
  ToolDefinition<typeof args> = {
    name:
      "agent-artifact-component-relations-remove-artifact-component-from-agent",
    description: `Remove Artifact Component from Agent`,
    annotations: {
      "title": "",
      "destructiveHint": true,
      "idempotentHint": false,
      "openWorldHint": false,
      "readOnlyHint": false,
    },
    args,
    tool: async (client, args, ctx) => {
      const [result, apiCall] =
        await agentArtifactComponentRelationsRemoveArtifactComponentFromAgent(
          client,
          args.request,
          { fetchOptions: { signal: ctx.signal } },
        ).$inspect();

      if (!result.ok) {
        return {
          content: [{ type: "text", text: result.error.message }],
          isError: true,
        };
      }

      const value = result.value;

      return formatResult(value, apiCall);
    },
  };
