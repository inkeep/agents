...
| Surface | Description | Depends On | Source Code |
|---|---|---|---|
| **GitHub Actions CI workflow** | Build/lint/typecheck/tests, format checks, env schema validation, and Knip run upstream on the agents-private monorepo pre-merge; no public-repo CI workflow. External PRs to `inkeep/agents` bridge back via `monorepo-pr-bridge.yml`. | Turborepo task graph, Env schema documentation enforcement, Knip dependency/exports analysis, Vitest workspace configs, Package-level Vitest configs & setup | Upstream: `agents-private/.github/workflows/public-agents-core-validation.yml`, `public-agents-extended-validation.yml` |
| **Cypress E2E workflow** | End-to-end UI tests run upstream on agents-private pre-merge; no public-repo Cypress workflow. | Local dev bootstrap (`pnpm setup-dev`), Local database stack (Docker Compose), Cypress E2E test harness | Upstream: `agents-private/.github/workflows/public-agents-cypress.yml`, `.github/composite-actions/public-agents-cypress-e2e/action.yml` |
