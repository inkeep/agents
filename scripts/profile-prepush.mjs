...
function findLatestSummary() {
  ...
  const files = readdirSync(turboRunsDir)
    ...
    .map((f) => ({
      ...
      mtime: new Date(
        readFileSync(join(turboRunsDir, f), 'utf8').match(/"startTime":\s*(\d+)/)?.[1] || 0
      ),
    }))
...
function analyzeSummary(summaryPath) {
  ...
  console.log(
    `${colors.bright}${colors.cyan}║           PRE-PUSH HOOK PERFORMANCE PROFILE                   ║${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.cyan}╚════════════════════════════════════════════════════════════════╝${colors.reset}\n`
  );
  ...
  console.log(
    `${colors.bright}Total Wall-Clock Time:${colors.reset} ${colors.yellow}${formatDuration(totalTime)}${colors.reset}`
  );
  console.log(
    `${colors.dim}Tasks Executed: ${summary.execution.attempted} | Cached: ${summary.execution.cached}${colors.reset}\n`
  );

  // Task type breakdown
  console.log(
    `${colors.bright}${colors.blue}═══ Time by Task Type (sum across packages) ═══${colors.reset}\n`
  );
  ...
  for (const [taskType, data] of sortedTypes) {
    ...
    console.log(
      `  ${colors.green}${taskType.padEnd(12)}${colors.reset} ${bar} ${formatDuration(data.total).padStart(8)} (${data.tasks.length} pkgs)`
    );
  }
  ...
  for (const task of buildTasks.slice(0, 5)) {
    ...
    console.log(
      `    • ${task.package.replace('@inkeep/', '')}: ${formatDuration(task.duration)} (${pct}% of total time)`
    );
  }
  ...
  if (docsBuild && docsBuild.duration > 30) {
    ...
    console.log(
      `     ${colors.dim}Impact: Save ~${formatDuration(docsBuild.duration)} (docs don't need validation on every push)${colors.reset}`
    );
    console.log(
      `     ${colors.dim}How: Add --filter='!@inkeep/agents-docs' to check:husky${colors.reset}\n`
    );
  }
  ...
  if (summary.execution.cached === 0) {
    ...
    console.log(
      `     ${colors.dim}Impact: Subsequent runs with unchanged code will be nearly instant${colors.reset}`
    );
  ...
  if (uiBuild && uiBuild.duration > 60) {
    console.log(
      `  ${colors.yellow}3. Consider excluding agents-manage-ui from pre-push${colors.reset}`
    );
    console.log(
      `     ${colors.dim}Impact: Save ~${formatDuration(uiBuild.duration)} if UI isn't changing${colors.reset}`
    );
    console.log(
      `     ${colors.dim}How: Add --filter='!@inkeep/agents-manage-ui' to check:husky${colors.reset}\n`
    );
  }

  console.log(`  ${colors.yellow}4. Use affected-only checks for faster iterations${colors.reset}`);
  console.log(
    `     ${colors.dim}Run: pnpm turbo check:husky --filter='...[origin/main]'${colors.reset}`
  );
  console.log(
    `     ${colors.dim}This only checks packages affected by your changes${colors.reset}\n`
  );
  ...
  if (excludePackages.length > 0) {
    ...
    console.log(
      `${colors.bright}${colors.green}═══ Suggested check:husky Command ═══${colors.reset}\n`
    );
...
function runProfile() {
  ...
  try {
    execSync(
      "TURBO_UI=stream pnpm turbo check:husky --filter='!agents-cookbook-templates' --summarize",
      {
        stdio: 'inherit',
        cwd: process.cwd(),
      }
...
if (!summaryPath) {
  console.error(
    `${colors.red}No turbo run summary found. Run with --run flag or run check:husky first.${colors.reset}`
  );
  process.exit(1);
}
