...
cmd_status() {
  ...
  if [ ! -d "$COMPANION_DIR" ]; then
    ...
    echo "  Run 'pnpm setup-dev:full' to set up optional services."
    exit 0
  fi
...
cmd_setup() {
  ...
  echo "  pnpm setup-dev:stop     Stop optional services"
  echo "  pnpm setup-dev:status   Show service status"
  echo "  pnpm setup-dev:reset    Nuke data + re-setup from scratch"
  echo ""
